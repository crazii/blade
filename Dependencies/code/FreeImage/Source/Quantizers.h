...
public:
    ...
    WuQuantizer(FreeImageMem* mem, FIBITMAP *dib);
	// Destructor
	~WuQuantizer();
	// Quantizer - Return value: quantized 8-bit (color palette) DIB
	FIBITMAP* Quantize(FreeImageMem* mem, int PaletteSize, int ReserveSize, RGBQUAD *ReservePalette);
};
...
public:
	/// Constructor
	NNQuantizer(FreeImageMem* mem, int PaletteSize);
	...
	*/
	FIBITMAP* Quantize(FreeImageMem* mem, FIBITMAP *dib, int ReserveSize, RGBQUAD *ReservePalette, int sampling = 1);
