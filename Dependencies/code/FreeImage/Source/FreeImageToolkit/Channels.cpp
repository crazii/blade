...
FreeImage_GetChannel(FreeImageMem* mem, FIBITMAP *src, FREE_IMAGE_COLOR_CHANNEL channel) {
	...
	if(image_type == FIT_BITMAP && ((bpp == 24) || (bpp == 32))) {
		...
		FIBITMAP *dst = FreeImage_Allocate(mem,width, height, 8) ;
		if(!dst) return NULL;
	...
	if((image_type == FIT_RGB16) ||  (image_type == FIT_RGBA16)) {
		...
		unsigned width  = FreeImage_GetWidth(src);
		unsigned height = FreeImage_GetHeight(src);
		FIBITMAP *dst = FreeImage_AllocateT(mem,FIT_UINT16, width, height) ;
		if(!dst) return NULL;
	...
	if((image_type == FIT_RGBF) ||  (image_type == FIT_RGBAF)) {
		...
		unsigned width  = FreeImage_GetWidth(src);
		unsigned height = FreeImage_GetHeight(src);
		FIBITMAP *dst = FreeImage_AllocateT(mem,FIT_FLOAT, width, height) ;
		if(!dst) return NULL;
...
*/
FIBITMAP * DLL_CALLCONV 
FreeImage_GetComplexChannel(FreeImageMem* mem, FIBITMAP *src, FREE_IMAGE_COLOR_CHANNEL channel) {
	unsigned x, y;
	double mag, phase;
	...
	if(FreeImage_GetImageType(src) == FIT_COMPLEX) {
		...
		unsigned width  = FreeImage_GetWidth(src);
		unsigned height = FreeImage_GetHeight(src);
		dst = FreeImage_AllocateT(mem,FIT_DOUBLE, width, height) ;
		if(!dst) return NULL;
