...
/// 4-bit
static BOOL Combine4(FreeImageMem* mem, FIBITMAP *dst_dib, FIBITMAP *src_dib, unsigned x, unsigned y, unsigned alpha);
/// 8-bit
...
static BOOL 
Combine4(FreeImageMem* mem, FIBITMAP *dst_dib, FIBITMAP *src_dib, unsigned x, unsigned y, unsigned alpha) {
	...
	BYTE *buffer = (BYTE *)FreeImage_Malloc(mem->temp_mem, src_line * sizeof(BYTE));
	if (buffer == NULL) {
		return FALSE;
	}
...
FreeImage_Copy(FreeImageMem* mem, FIBITMAP *src, int left, int top, int right, int bottom) {
	...
	FIBITMAP *dst = 
		FreeImage_AllocateT(mem,FreeImage_GetImageType(src), 
							dst_width, 
							dst_height, 
							bpp, 
...
*/
BOOL DLL_CALLCONV 
FreeImage_Paste(FreeImageMem* mem, FIBITMAP *dst, FIBITMAP *src, int left, int top, int alpha) {
	BOOL bResult = FALSE;
	...
	if(image_type == FIT_BITMAP) {
		...
		if(bpp_dst == bpp_src) {
			...
			switch(bpp_dst) {
				case 4:
					clone = FreeImage_ConvertTo4Bits(&tmem,src);
					break;
				case 8:
					clone = FreeImage_ConvertTo8Bits(&tmem,src);
					break;
				case 16:
					if (isRGB565) {
						clone = FreeImage_ConvertTo16Bits565(&tmem,src);
					} else {
						// includes case where all the masks are 0
						clone = FreeImage_ConvertTo16Bits555(&tmem,src);
					}
					break;
				case 24:
					clone = FreeImage_ConvertTo24Bits(&tmem,src);
					break;
				case 32:
					clone = FreeImage_ConvertTo32Bits(&tmem,src);
					break;
				default:
					return FALSE;
			}
		...
		switch(FreeImage_GetBPP(dst)) {
			case 1:
				...
				break;
			case 4:
				bResult = Combine4(&tmem, dst, clone, (unsigned)left, (unsigned)top, (unsigned)alpha);
				break;
			case 8:
