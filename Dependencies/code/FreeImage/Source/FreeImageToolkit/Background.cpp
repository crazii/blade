...
FreeImage_AllocateExT(FreeImageMem* mem, FREE_IMAGE_TYPE type, int width, int height, int bpp, const void *color, int options, const RGBQUAD *palette, unsigned red_mask, unsigned green_mask, unsigned blue_mask) {

	FIBITMAP *bitmap = FreeImage_AllocateT(mem,type, width, height, bpp, red_mask, green_mask, blue_mask);
 ...
 */
FIBITMAP * DLL_CALLCONV
FreeImage_AllocateEx(FreeImageMem* mem, int width, int height, int bpp, const RGBQUAD *color, int options, const RGBQUAD *palette, unsigned red_mask, unsigned green_mask, unsigned blue_mask) {
	return FreeImage_AllocateExT(mem,FIT_BITMAP, width, height, bpp, ((void *)color), options, palette, red_mask, green_mask, blue_mask);
}
 ...
 */
FIBITMAP * DLL_CALLCONV
FreeImage_EnlargeCanvas(FreeImageMem* mem, FIBITMAP *src, int left, int top, int right, int bottom, const void *color, int options) {
	...
	// NULL in this case.
	if ((left <= 0) && (right <= 0) && (top <= 0) && (bottom <= 0)) {
		return FreeImage_Copy(mem,src, -left, -top,	width + right, height + bottom);
	}
	...
	FIBITMAP *dst = FreeImage_AllocateExT(mem,
	...
	if ((type == FIT_BITMAP) && (bpp <= 4)) {
		...
		FIBITMAP *copy = FreeImage_Copy(&tmem,src,
			((left >= 0) ? 0 : -left),
			((top >= 0) ? 0 : -top),
		...
		if (!FreeImage_Paste(mem,dst, copy,
				((left <= 0) ? 0 : left),
				((top <= 0) ? 0 : top), 256)) {
			FreeImage_Unload(&tmem,copy);
			FreeImage_Unload(mem,dst);
			return NULL;
		}
	...
	FIICCPROFILE *dst_profile = FreeImage_CreateICCProfile(mem, dst, src_profile->data, src_profile->size); 
