...
*/
BOOL DLL_CALLCONV 
FreeImage_FlipHorizontal(FreeImageMem* mem, FIBITMAP *src) {
	...
	BYTE *new_bits = (BYTE*)FreeImage_AlignMalloc( mem->temp_mem, line * sizeof(BYTE), FIBITMAP_ALIGNMENT);
	...
	FreeImage_AlignFree(mem->temp_mem,new_bits);
...
BOOL DLL_CALLCONV 
FreeImage_FlipVertical(FreeImageMem* mem, FIBITMAP *src) {
	BYTE *From, *Mid;
	...
	// copy between aligned memories
	Mid = (BYTE*)FreeImage_AlignMalloc(mem->temp_mem, pitch * sizeof(BYTE), FIBITMAP_ALIGNMENT);
	if (!Mid) return FALSE;
	...
	FreeImage_AlignFree(mem->temp_mem, Mid);
