...
*/
static bool	
SamplesToCoefficients(FreeImageMem* mem, double *Image, long Width, long Height, long spline_degree) {
	double	*Line;
	double	Pole[2];
	long	NbPoles;
	long	x, y;
	...
	Line = (double *)FreeImage_Malloc(mem->temp_mem,Width * sizeof(double));
	if (Line == NULL) {
	...
	FreeImage_Free(mem->temp_mem,Line);

	// in-place separable process, along y 
	Line = (double *)FreeImage_Malloc(mem->temp_mem,Height * sizeof(double));
	if (Line == NULL) {
...
*/
static FIBITMAP * 
Rotate8Bit(FreeImageMem* mem, FIBITMAP *dib, double angle, double x_shift, double y_shift, double x_origin, double y_origin, long spline_degree, BOOL use_mask) {
	...
	FIBITMAP *dst = FreeImage_Allocate(mem,width, height, bpp);
	if(!dst)
		return NULL;
	...
	ImageRasterArray = (double*)FreeImage_Malloc(mem->temp_mem,width * height * sizeof(double));
	if(!ImageRasterArray) {
	...
	bResult = SamplesToCoefficients(mem,ImageRasterArray, width, height, spline);
	if(!bResult) {
		FreeImage_Unload(mem,dst);
		FreeImage_Free(mem->temp_mem,ImageRasterArray);
		return NULL;
	}
...
FreeImage_RotateEx(FreeImageMem* mem, FIBITMAP *dib, double angle, double x_shift, double y_shift, double x_origin, double y_origin, BOOL use_mask) {
	...
	try {
		...
		if(bpp == 8) {
			FIBITMAP *dst_8 = Rotate8Bit(mem,dib, angle, x_shift, y_shift, x_origin, y_origin, ROTATE_CUBIC, use_mask);
			if(dst_8) {
				// copy metadata from src to dst
				FreeImage_CloneMetadata(mem,dst_8, dib);
			}
			return dst_8;
		}
		if((bpp == 24) || (bpp == 32)) {
			...
			int height = FreeImage_GetHeight(dib);
			if( bpp == 24 ) {
				dst = FreeImage_Allocate(mem,width, height, bpp, FI_RGBA_RED_MASK, FI_RGBA_GREEN_MASK, FI_RGBA_BLUE_MASK);
			} else {
				dst = FreeImage_Allocate(mem,width, height, bpp, FI_RGBA_RED_MASK, FI_RGBA_GREEN_MASK, FI_RGBA_BLUE_MASK);
			}
			if(!dst) throw(1);
			...
			src8 = FreeImage_Allocate(&tmem,width, height, 8);
			if(!src8) throw(1);
			...
			for(channel = 0; channel < nb_channels; channel++) {
				...
				// process channel
				dst8 = Rotate8Bit(&tmem,src8, angle, x_shift, y_shift, x_origin, y_origin, ROTATE_CUBIC, use_mask);
				if(!dst8) throw(1);
				...
				FreeImage_Unload(&tmem,dst8);
			}

			FreeImage_Unload(&tmem,src8);
		...
		if(src8) FreeImage_Unload(&tmem,src8);
		if(dst8) FreeImage_Unload(&tmem,dst8);
