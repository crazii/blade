...
static FIBITMAP* Rotate90(FreeImageMem* mem, FIBITMAP *src);
static FIBITMAP* Rotate180(FreeImageMem* mem, FIBITMAP *src);
static FIBITMAP* Rotate270(FreeImageMem* mem, FIBITMAP *src);
static FIBITMAP* Rotate45(FreeImageMem* mem, FIBITMAP *src, double dAngle, const void *bkcolor);
static FIBITMAP* RotateAny(FreeImageMem* mem, FIBITMAP *src, double dAngle, const void *bkcolor);
...
*/
static FIBITMAP* 
Rotate90(FreeImageMem* mem,FIBITMAP *src) {
	int x, y, y2;
	...
	FIBITMAP *dst = FreeImage_AllocateT(mem,image_type, dst_width, dst_height, bpp);
...
*/
static FIBITMAP* 
Rotate180(FreeImageMem* mem,FIBITMAP *src) {
	int x, y, k, pos;
...
*/
static FIBITMAP* 
Rotate270(FreeImageMem* mem,FIBITMAP *src) {
	int x, x2, y, dlineup;
...
*/
static FIBITMAP* 
Rotate45(FreeImageMem* mem,FIBITMAP *src, double dAngle, const void *bkcolor) {
	...
	FIBITMAP *dst1 = FreeImage_AllocateT(&tmem,image_type, width_1, height_1, bpp);
	if(NULL == dst1) {
		return NULL;
	}
	...
	FIBITMAP *dst2 = FreeImage_AllocateT(&tmem,image_type, width_2, height_2, bpp);
	if(NULL == dst2) {
		FreeImage_Unload(&tmem,dst1);
		return NULL;
	}
	...
	FIBITMAP *dst3 = FreeImage_AllocateT(&tmem,image_type, width_3, height_3, bpp);
	if(NULL == dst3) {
		FreeImage_Unload(&tmem,dst2);
		return NULL;
	}
...
*/
static FIBITMAP* 
RotateAny(FreeImageMem* mem, FIBITMAP *src, double dAngle, const void *bkcolor) {
	if(NULL == src) {
		return NULL;
	}
	...
	FIBITMAP *dst = Rotate45(mem,image, dAngle, bkcolor);

	if(src != image) {
	// Middle image was required, free it now.
	FreeImage_Unload(&tmem,image);
	}
...
FreeImage_Rotate(FreeImageMem* mem,FIBITMAP *dib, double angle, const void *bkcolor) {
		...
		switch(image_type) {
			case FIT_BITMAP:
				if(bpp == 1) {
					...
					FIBITMAP *dst = RotateAny(mem,dib, angle, bkcolor);
					if(!dst) throw(1);
