...
*/
CWeightsTable::CWeightsTable(FreeImageMem* mem, CGenericFilter *pFilter, DWORD uDstSize, DWORD uSrcSize) {
	DWORD u;
	double dWidth;
	double dFScale = 1.0;
	...
	m_WeightTable = (Contribution*)FreeImage_Malloc(mem->temp_mem, m_LineLength * sizeof(Contribution));
	for(u = 0 ; u < m_LineLength ; u++) {
		// allocate contributions for every pixel
		m_WeightTable[u].Weights = (double*)FreeImage_Malloc(mem->temp_mem, m_WindowSize * sizeof(double));
	}
...
CWeightsTable::~CWeightsTable() {
	for(DWORD u = 0; u < m_LineLength; u++) {
		...
		FreeImage_Free(memobject->temp_mem, m_WeightTable[u].Weights);
	}
	// free list of pixels contributions
	FreeImage_Free(memobject->temp_mem, m_WeightTable);
}
...
FIBITMAP* CResizeEngine::scale(FreeImageMem* mem,FIBITMAP *src, unsigned dst_width, unsigned dst_height) { 
	...
	FIBITMAP *dst = FreeImage_AllocateT(mem, image_type, dst_width, dst_height, bpp, redMask, greenMask, blueMask);
	if(!dst) return NULL;
	...
	if(dst_width*src_height <= dst_height*src_width) {
		...
		FIBITMAP *tmp = FreeImage_AllocateT(&tmem,image_type, dst_width, src_height, bpp, redMask, greenMask, blueMask);
		if(!tmp) {
			FreeImage_Unload(mem,dst);
			return NULL;
		}

		// scale source image horizontally into temporary image
		horizontalFilter(&tmem, src, src_width, src_height, tmp, dst_width, src_height);

		// scale temporary image vertically into result image    
		verticalFilter(&tmem, tmp, dst_width, src_height, dst, dst_width, dst_height);

		// free temporary image
		FreeImage_Unload(&tmem,tmp);
		...
		// allocate a temporary image
		FIBITMAP *tmp = FreeImage_AllocateT(mem,image_type, src_width, dst_height, bpp, redMask, greenMask, blueMask);
		if(!tmp) {
			FreeImage_Unload(mem,dst);
			return NULL;
		}
		...
		verticalFilter(&tmem, src, src_width, src_height, tmp, src_width, dst_height);

		// scale temporary image horizontally into result image    
		horizontalFilter(&tmem, tmp, src_width, dst_height, dst, dst_width, dst_height);
...
void CResizeEngine::horizontalFilter(FreeImageMem* mem, FIBITMAP *src, unsigned src_width, unsigned src_height, FIBITMAP *dst, unsigned dst_width, unsigned dst_height) { 
	if(dst_width == src_width) {
		...
		CWeightsTable weightsTable(mem, m_pFilter, dst_width, src_width); 
...
void CResizeEngine::verticalFilter(FreeImageMem* mem, FIBITMAP *src, unsigned src_width, unsigned src_height, FIBITMAP *dst, unsigned dst_width, unsigned dst_height) { 
	if(src_height == dst_height) {
		...
		// allocate and calculate the contributions
		CWeightsTable weightsTable(mem, m_pFilter, dst_height, src_height); 
