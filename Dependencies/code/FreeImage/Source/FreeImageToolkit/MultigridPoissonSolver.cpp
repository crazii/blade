...
used at each level.
*/
static BOOL fmg_mglin(FreeImageMem* mem, FIBITMAP *U, int n, int ncycle) {
...
#define _CREATE_ARRAY_GRID_(array, array_size) \
	array = (FIBITMAP**)FreeImage_Malloc(tmem.temp_mem,array_size * sizeof(FIBITMAP*));\
	if(!array) throw(1);\
...
#define _FREE_ARRAY_GRID_(array, array_size) \
	if(NULL != array) {\
		for(int k = 0; k < array_size; k++) {\
			if(NULL != array[k]) {\
				FreeImage_Unload(&tmem,array[k]); array[k] = NULL;\
			}\
		}\
		FreeImage_Free(tmem.temp_mem,array);\
	}
...
// --------------------------------------------------------------------------
	...
	try {
		...
		IRHO[ngrid] = FreeImage_AllocateT(&tmem,FIT_FLOAT, nn, nn);
		...
		IU[0] = FreeImage_AllocateT(&tmem,FIT_FLOAT, nn, nn);
		if(!IU[0]) throw(1);
		IRHS[0] = FreeImage_AllocateT(&tmem,FIT_FLOAT, nn, nn);
		if(!IRHS[0]) throw(1);

		// initial solution on coarsest grid
		fmg_solve(IU[0], IRHO[0]);
		// irho[0] no longer needed ...
		FreeImage_Unload(&tmem,IRHO[0]); IRHO[0] = NULL;
		...
		for (j = 1; j < ngrid; j++) {
			...
			IU[j] = FreeImage_AllocateT(&tmem,FIT_FLOAT, nn, nn);
			if(!IU[j]) throw(1);
			IRHS[j] = FreeImage_AllocateT(&tmem,FIT_FLOAT, nn, nn);
			if(!IRHS[j]) throw(1);
			IRES[j] = FreeImage_AllocateT(&tmem,FIT_FLOAT, nn, nn);
			if(!IRES[j]) throw(1);
...
FreeImage_MultigridPoissonSolver(FreeImageMem* mem, FIBITMAP *Laplacian, int ncycle) {
	...
	FIBITMAP *I = FreeImage_AllocateT(&tmem,FIT_FLOAT, size, size);
	if(!I) return NULL;

	// copy Laplacian into I and shift pixels to create a boundary
	FreeImage_Paste(&tmem,I, Laplacian, 1, 1, 255);

	// solve the PDE equation
	fmg_mglin(mem,I, size, ncycle);

	// shift pixels back
	FIBITMAP *U = FreeImage_Copy(mem, I, 1, 1, width + 1, height + 1);
	FreeImage_Unload(&tmem,I);

	// remap pixels to [0..1]
	NormalizeY(U, 0, 1);

	// copy metadata from src to dst
	FreeImage_CloneMetadata(mem,U, Laplacian);
