...
FreeImage_ApplyColorMapping(FreeImageMem* mem, FIBITMAP *dib, RGBQUAD *srccolors, RGBQUAD *dstcolors, unsigned count, BOOL ignore_alpha, BOOL swap) {
	unsigned result = 0;
	...
	switch (bpp) {
		...
		case 16: {
			WORD *src16 = (WORD *)FreeImage_Malloc(mem->temp_mem, sizeof(WORD) * count);
			if (NULL == src16) {
				return 0;
			}

			WORD *dst16 = (WORD *)FreeImage_Malloc(mem->temp_mem, sizeof(WORD) * count);
			if (NULL == dst16) {
				FreeImage_Free(mem->temp_mem, src16);
				return 0;
			}
			...
			FreeImage_Free(mem->temp_mem, src16);
			FreeImage_Free(mem->temp_mem, dst16);
			return result;
		}
		case 24: {
			unsigned height = FreeImage_GetHeight(dib);
 ...
 */
unsigned DLL_CALLCONV
FreeImage_SwapColors(FreeImageMem* mem, FIBITMAP *dib, RGBQUAD *color_a, RGBQUAD *color_b, BOOL ignore_alpha) {
	return FreeImage_ApplyColorMapping(mem, dib, color_a, color_b, 1, ignore_alpha, TRUE);
}
