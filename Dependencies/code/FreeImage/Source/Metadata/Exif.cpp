...
*/
static void 
processMakerNote(FreeImageMem* mem, FIBITMAP *dib, char *pval, BOOL msb_order, DWORD *subdirOffset, TagLib::MDMODEL *md_model) {
	FITAG *tagMake = NULL;
...
*/
static BOOL 
processCanonMakerNoteTag(FreeImageMem* mem, FIBITMAP *dib, FITAG *tag) {
	char defaultKey[16];
	DWORD startIndex = 0;
	...
	switch(tag_id) {
		...
		default:
		{
			...
			// store the tag
			if(key) {
				FreeImage_SetMetadata(FIMD_EXIF_MAKERNOTE, mem, dib, key, tag);
			}
	...
	// create a tag
	FITAG *canonTag = FreeImage_CreateTag(mem);
    ...
    for (DWORD i = startIndex; i < FreeImage_GetTagCount(tag); i++) {
		...
		FreeImage_SetTagValue(mem, canonTag, &pvalue[i]);

		// get the tag key and description
		const char *key = s.getTagFieldName(TagLib::EXIF_MAKERNOTE_CANON, tag_id, defaultKey);
		FreeImage_SetTagKey(mem, canonTag, key);
		const char *description = s.getTagDescription(TagLib::EXIF_MAKERNOTE_CANON, tag_id);
		FreeImage_SetTagDescription(mem, canonTag, description);

		// store the tag
		if(key) {
			FreeImage_SetMetadata(FIMD_EXIF_MAKERNOTE, mem, dib, key, canonTag);
		}
	}

	// delete the tag
	FreeImage_DeleteTag(mem, canonTag);
...
*/
static void 
processExifTag(FreeImageMem* mem, FIBITMAP *dib, FITAG *tag, char *pval, BOOL msb_order, TagLib::MDMODEL md_model) {
	char defaultKey[16];
	int n;
	DWORD i;

	// allocate a buffer to store the tag value
	BYTE *exif_value = (BYTE*)FreeImage_Malloc(mem->temp_mem, FreeImage_GetTagLength(tag) * sizeof(BYTE) );
	if(NULL == exif_value) {
		// out of memory ...
		return;
	}
	...
	if(md_model == TagLib::EXIF_MAKERNOTE_CANON) {
		...
		// store the tag
		if(key) {
			FreeImage_SetMetadata(s.getFreeImageModel(md_model), mem, dib, key, tag);
		}
	}
	

	// free the temporary buffer
	FreeImage_Free(mem->temp_mem,exif_value);
...
*/
static BOOL 
jpeg_read_exif_dir(FreeImageMem* mem, FIBITMAP *dib, const BYTE *tiffp, unsigned long offset, unsigned int length, BOOL msb_order) {
	WORD de, nde;
	...
	*/
		...
		for(; de < nde; de++) {
			...
			if(isIFDOffset)	{
				...
				if (FreeImage_GetTagID(tag) == TAG_MAKER_NOTE) {
					processMakerNote(mem, dib, pval, msb_order, &sub_offset, &next_mdmodel);
				...
				if((sub_offset < (DWORD) length) && (next_mdmodel != TagLib::UNKNOWN)) {
					...
					processExifTag(mem, dib, tag, pval, msb_order, md_model);
				}			
	...
	FIMEMORY* hmem = FreeImage_OpenMemory(&tmem, const_cast<BYTE*>(thLocation), thSize);
	//FIBITMAP* thumbnail = FreeImage_LoadFromMemory(FIF_JPEG, &tmem, hmem);
	FIBITMAP* thumbnail = FreeImage_LoadFromMemory(FIF_BMP, &tmem, hmem);
	FreeImage_CloseMemory(&tmem, hmem);
	
	// store the thumbnail
	FreeImage_SetThumbnail(mem, dib, thumbnail);
	// then delete it
...
*/
BOOL  
jpeg_read_exif_profile(FreeImageMem* mem, FIBITMAP *dib, const BYTE *dataptr, unsigned int datalen) {
	...
	if(memcmp(exif_signature, profile, sizeof(exif_signature)) == 0) {
		...
		*/
		...
		return jpeg_read_exif_dir(mem,dib, profile, first_offset, length, bMotorolaOrder);
	}
