...
// JPEG Exif profile
BOOL jpeg_read_exif_profile(FreeImageMem* mem, FIBITMAP *dib, const BYTE *dataptr, unsigned int datalen);

// JPEG / TIFF IPTC profile
BOOL read_iptc_profile(FIBITMAP *dib, const BYTE *dataptr, unsigned int datalen);
BOOL write_iptc_profile(FreeImageMem* mem, FIBITMAP *dib, BYTE **profile, unsigned *profile_size);
