...
*/
static BOOL 
tiff_read_exif_tag(FreeImageMem* mem, TIFF *tif, TagLib::MDMODEL md_model, FIBITMAP *dib, TagLib& tagLib, TIFFDirectory *td, ttag_t tag) {
	...
	if(fip->field_passcount) { //<- "passcount" means "returns count"
		...
		if ((fip->field_type == TIFF_ASCII
			...
			raw_data = FreeImage_Malloc( mem->temp_mem, value_size * value_count);
			mem_alloc = 1;
			int ok = FALSE;
...
*/
			...
			if(ok != 1) {
				FreeImage_Free(mem->temp_mem, raw_data);
				return TRUE;
			}
		}
	}
	...
	switch(fip->field_type) {
		...
		case TIFF_RATIONAL: {
			...
			DWORD *rvalue = (DWORD*)FreeImage_Malloc(mem->temp_mem, * value_count * sizeof(DWORD));
			...
			FreeImage_Free(mem->temp_mem, rvalue);
		}
		break;

		case TIFF_SRATIONAL: {
			// LibTIFF converts rational to floats : reconvert floats to rationals
			LONG *rvalue = (LONG*)FreeImage_Malloc(mem->temp_mem, 2 * value_count * sizeof(LONG));
			for(uint32 i = 0; i < value_count; i++) {
				float *fv = (float*)raw_data;
				FIRational rational(fv[i]);
				rvalue[2*i] = rational.getNumerator();
