...
*/
BOOL 
read_iptc_profile(FreeImageMem* mem, FIBITMAP *dib, const BYTE *dataptr, unsigned int datalen) {
	char defaultKey[16];
    ...
    while (offset < length) {
		...
		BYTE *iptc_value = (BYTE*)FreeImage_Malloc(mem->temp_mem,(tagByteCount + 1) * sizeof(BYTE));
		...
		switch (tag_id) {
			case TAG_RECORD_VERSION:
			{
				...
				*pvalue = (short)((profile[offset] << 8) | profile[offset + 1]);
				FreeImage_SetTagValue(mem, tag, pvalue);
				break;
			}
			...
			default:
			{
				...
				FreeImage_SetTagValue(mem, tag, (char*)&iptc_value[0]);
				break;
			}
		}
		...
		if(tag_id == TAG_SUPPLEMENTAL_CATEGORIES) {
			...
			// store the tag
			if(key) {
				FreeImage_SetMetadata(FIMD_IPTC, mem, dib, key, tag);
			}
		}

		FreeImage_Free(mem->temp_mem, iptc_value);
	...
	if(Keywords.length()) {
		...
		FreeImage_SetTagKey(mem, tag, tag_lib.getTagFieldName(TagLib::IPTC, TAG_KEYWORDS, defaultKey));
		FreeImage_SetTagDescription(mem, tag, tag_lib.getTagDescription(TagLib::IPTC, TAG_KEYWORDS));
		FreeImage_SetTagLength(tag, (DWORD)Keywords.length());
		FreeImage_SetTagCount(tag, (DWORD)Keywords.length());
		FreeImage_SetTagValue(mem, tag, (char*)Keywords.c_str());
		FreeImage_SetMetadata(FIMD_IPTC, mem, dib, FreeImage_GetTagKey(tag), tag);
	}
	...
	if(SupplementalCategory.length()) {
		...
		FreeImage_SetTagKey(mem, tag, tag_lib.getTagFieldName(TagLib::IPTC, TAG_SUPPLEMENTAL_CATEGORIES, defaultKey));
		FreeImage_SetTagDescription(mem, tag, tag_lib.getTagDescription(TagLib::IPTC, TAG_SUPPLEMENTAL_CATEGORIES));
		FreeImage_SetTagLength(tag, (DWORD)SupplementalCategory.length());
		FreeImage_SetTagCount(tag, (DWORD)SupplementalCategory.length());
		FreeImage_SetTagValue(mem, tag, (char*)SupplementalCategory.c_str());
		FreeImage_SetMetadata(FIMD_IPTC, mem, dib, FreeImage_GetTagKey(tag), tag);
	}
...
static BYTE* 
append_iptc_tag(FreeImageMem* mem, BYTE *profile, unsigned *profile_size, WORD id, DWORD length, const void *value) {
	BYTE *buffer = NULL;

	// calculate the new buffer size
	size_t buffer_size = (5 + *profile_size + length) * sizeof(BYTE);
	buffer = (BYTE*)FreeImage_Malloc(mem->temp_mem, buffer_size);
	if(!buffer)
		return NULL;
	...
	if(NULL == profile)	{
		...
		*profile_size += (5 + length);
		FreeImage_Free(mem->temp_mem, profile);
	}
...
*/
BOOL 
write_iptc_profile(FreeImageMem* mem, FIBITMAP *dib, BYTE **profile, unsigned *profile_size) {
	FITAG *tag = NULL;
	...
	mdhandle = FreeImage_FindFirstMetadata(mem, FIMD_IPTC, dib, &tag);
	...
	if(mdhandle) {
			...
			switch(tag_id) {
				...
				case TAG_KEYWORDS:
					if(FreeImage_GetTagType(tag) == FIDT_ASCII) {
						...
						for(int i = 0; i < (int)output.size(); i++) {
							...
							buffer = append_iptc_tag(mem, buffer, &buffer_size, tag_id, (DWORD)tag_value.length(), tag_value.c_str());
						}
				...
				case TAG_URGENCY:
					if(FreeImage_GetTagType(tag) == FIDT_ASCII) {
						...
						buffer = append_iptc_tag(mem, buffer, &buffer_size, tag_id, length, FreeImage_GetTagValue(tag));
					}
					break;
		...
		buffer = append_iptc_tag(mem, buffer, &buffer_size, TAG_RECORD_VERSION, sizeof(version), &version);
