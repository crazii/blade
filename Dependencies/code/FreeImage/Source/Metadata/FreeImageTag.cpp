...
FITAG * DLL_CALLCONV 
FreeImage_CreateTag(FreeImageMem* mem) {
	FITAG *tag = (FITAG *)FreeImage_Malloc(mem->normal_mem,sizeof(FITAG));

	if (tag != NULL) {
		unsigned tag_size = sizeof(FITAGHEADER); 
		tag->data = (BYTE *)FreeImage_Malloc(mem->normal_mem,tag_size * sizeof(BYTE));
		if (tag->data != NULL) {
			memset(tag->data, 0, tag_size);
			return tag;
		}
		FreeImage_Free(mem->normal_mem, tag);
	}
...
void DLL_CALLCONV 
FreeImage_DeleteTag(FreeImageMem* mem, FITAG *tag) {
	if (NULL != tag) {	
		if (NULL != tag->data) {
			FITAGHEADER *tag_header = (FITAGHEADER *)tag->data;
			// delete tag members
			FreeImage_Free(mem->normal_mem, tag_header->key); 
			FreeImage_Free(mem->normal_mem, tag_header->description); 
			FreeImage_Free(mem->normal_mem, tag_header->value);
			// delete the tag
			FreeImage_Free(mem->normal_mem, tag->data);
		}
		// and the wrapper
		FreeImage_Free(mem->normal_mem,tag);
	}
...
FITAG * DLL_CALLCONV 
FreeImage_CloneTag(FreeImageMem* mem, FITAG *tag) {
	if(!tag) return NULL;

	// allocate a new tag
	FITAG *clone = FreeImage_CreateTag(mem);
	...
	// tag key
	if(src_tag->key) {
		dst_tag->key = (char*)FreeImage_Malloc(mem->normal_mem,(strlen(src_tag->key) + 1) * sizeof(char));
		strcpy(dst_tag->key, src_tag->key);
	}
	// tag description
	if(src_tag->description) {
		dst_tag->description = (char*)FreeImage_Malloc(mem->normal_mem,(strlen(src_tag->description) + 1) * sizeof(char));
	...
	switch(dst_tag->type) {
		case FIDT_ASCII:
			dst_tag->value = (char*)FreeImage_Malloc(mem->normal_mem,(strlen((char*)src_tag->value) + 1) * sizeof(char));
			strcpy((char*)dst_tag->value, (char*)src_tag->value);
			break;
		default:
			dst_tag->value = (BYTE*)FreeImage_Malloc(mem->normal_mem,src_tag->length * sizeof(BYTE));
...
BOOL DLL_CALLCONV 
FreeImage_SetTagKey(FreeImageMem* mem, FITAG *tag, const char *key) {
	if(tag && key) {
		FITAGHEADER *tag_header = (FITAGHEADER *)tag->data;
		if(tag_header->key) FreeImage_Free(mem->normal_mem, tag_header->key);
		tag_header->key = (char*)FreeImage_Malloc(mem->normal_mem, strlen(key) + 1);
...
BOOL DLL_CALLCONV 
FreeImage_SetTagDescription(FreeImageMem* mem, FITAG *tag, const char *description) {
	if(tag && description) {
		FITAGHEADER *tag_header = (FITAGHEADER *)tag->data;
		if(tag_header->description)FreeImage_Free(mem->normal_mem, tag_header->description);
		tag_header->description = (char*)FreeImage_Malloc(mem->normal_mem, strlen(description) + 1);
...
BOOL DLL_CALLCONV 
FreeImage_SetTagValue(FreeImageMem* mem, FITAG *tag, const void *value) {
	if(tag && value) {
		FITAGHEADER *tag_header = (FITAGHEADER *)tag->data;
		// first, check the tag
		...
		switch(tag_header->type) {
			case FIDT_ASCII:
			{
				tag_header->value = (char*)FreeImage_Malloc(mem->normal_mem, (tag_header->length + 1) * sizeof(char));
				if(!tag_header->value) {
					return FALSE;
				}
			...
			default:
				tag_header->value = FreeImage_Malloc(mem->normal_mem, tag_header->length * sizeof(BYTE));
				if(!tag_header->value) {
					return FALSE;
				}
