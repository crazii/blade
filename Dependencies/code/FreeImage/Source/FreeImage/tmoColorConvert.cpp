...
*/
FIBITMAP* 
ClampConvertRGBFTo24(FreeImageMem* mem, FIBITMAP *src) {
	...
	FIBITMAP *dst = FreeImage_Allocate(mem, width, height, 24, FI_RGBA_RED_MASK, FI_RGBA_GREEN_MASK, FI_RGBA_BLUE_MASK);
	if(!dst) return NULL;
...
*/
FIBITMAP*  
ConvertRGBFToY(FreeImageMem* mem, FIBITMAP *src) {
	if(FreeImage_GetImageType(src) != FIT_RGBF)
		return FALSE;
