...
FreeImage_OpenMemory(FreeImageMem* mem, BYTE *data, DWORD size_in_bytes) {
	// allocate a memory handle
	FIMEMORY *stream = (FIMEMORY*)FreeImage_Malloc(mem->temp_mem, sizeof(FIMEMORY));
	if(stream) {
		stream->data = (BYTE*)FreeImage_Malloc(mem->temp_mem, sizeof(FIMEMORYHEADER));
		...
		FreeImage_Free(mem->temp_mem, stream);
	}
...
void DLL_CALLCONV
FreeImage_CloseMemory(FreeImageMem* mem, FIMEMORY *stream) {
	if(stream && stream->data) {
		FIMEMORYHEADER *mem_header = (FIMEMORYHEADER*)(stream->data);
		if(mem_header->delete_me) {
			FreeImage_Free(mem->temp_mem, mem_header->data);
		}
		FreeImage_Free(mem->temp_mem, mem_header);
		FreeImage_Free(mem->temp_mem, stream);
	}
}
...
FreeImage_LoadFromMemory(FREE_IMAGE_FORMAT fif, FreeImageMem* mem, FIMEMORY *stream, int flags) {
	if (stream && stream->data) {
		FreeImageIO io;
		SetMemoryIO(&io);

		return FreeImage_LoadFromHandle(fif, &io, mem, (fi_handle)stream, flags);
	}
...
BOOL DLL_CALLCONV
FreeImage_SaveToMemory(FREE_IMAGE_FORMAT fif, FreeImageMem* mem, FIBITMAP *dib, FIMEMORY *stream, int flags) {
	if (stream) {
		FreeImageIO io;
		SetMemoryIO(&io);
		...
		if(mem_header->delete_me == TRUE) {
			return FreeImage_SaveToHandle(fif, dib, &io, mem, (fi_handle)stream, flags);
		} else {
