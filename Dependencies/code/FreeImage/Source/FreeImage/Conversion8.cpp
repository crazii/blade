...
FreeImage_ConvertTo8Bits(FreeImageMem* mem, FIBITMAP *dib) {
...
FIBITMAP * DLL_CALLCONV
FreeImage_ConvertToGreyscale(FreeImageMem* mem, FIBITMAP *dib) {
	if(!FreeImage_HasPixels(dib)) return NULL;
	...
	if((color_type == FIC_PALETTE) || (color_type == FIC_MINISWHITE)) {
		...
		BYTE *buffer = (BYTE*)FreeImage_Malloc(mem->temp_mem, CalculatePitch(CalculateLine(width, 24)) * sizeof(BYTE) );
		if(NULL == buffer) {
			FreeImage_Unload(mem, new_dib);
			return NULL;
		}
	...
	return FreeImage_ConvertTo8Bits(mem, dib);
}
