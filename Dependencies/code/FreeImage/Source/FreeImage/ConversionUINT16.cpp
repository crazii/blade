...
FreeImage_ConvertToUINT16(FreeImageMem* mem, FIBITMAP *dib) {
	FIBITMAP *src = NULL;
	FIBITMAP *dst = NULL;
	...
	switch(src_type) {
		case FIT_BITMAP:
		{
			...
			if((FreeImage_GetBPP(dib) == 8) && (FreeImage_GetColorType(dib) == FIC_MINISBLACK)) {
				...
				src = FreeImage_ConvertToGreyscale(&tmem, dib);
				if(!src) return NULL;
			}
			break;
		}
	...
	dst = FreeImage_AllocateT(mem, FIT_UINT16, width, height);
	if(!dst) {
		if(src != dib) {
			FreeImage_Unload(&tmem, src);
		}
		return NULL;
	}
