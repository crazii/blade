...
// png memory interface
static png_voidp PngMemAlloc(png_structp png_ptr, size_t size){
	return FreeImage_Malloc( (FreeImageMemType*)png_ptr->mem_ptr,size);
}

static void PngMemFree(png_structp png_ptr, png_voidp ptr){
	FreeImage_Free( (FreeImageMemType*)png_ptr->mem_ptr,ptr);
}
...
static BOOL 
ReadMetadata(FreeImageMem* mem, png_structp png_ptr, png_infop info_ptr, FIBITMAP *dib) {
	// XMP keyword
	...
	if(png_get_text(png_ptr, info_ptr, &text_ptr, &num_text) > 0) {
		for(int i = 0; i < num_text; i++) {
			...
			FreeImage_SetTagValue(mem, tag, text_ptr[i].text);

			if(strcmp(text_ptr[i].key, g_png_xmp_keyword) == 0) {
				// store the tag as XMP
				FreeImage_SetTagKey(mem, tag, g_TagLib_XMPFieldName);
				FreeImage_SetMetadata(FIMD_XMP, mem, dib, FreeImage_GetTagKey(tag), tag);
			} else {
				// store the tag as a comment
				FreeImage_SetTagKey(mem, tag, text_ptr[i].key);
				FreeImage_SetMetadata(FIMD_COMMENTS, mem, dib, FreeImage_GetTagKey(tag), tag);
			}
...
static BOOL 
WriteMetadata(FreeImageMem* mem, png_structp png_ptr, png_infop info_ptr, FIBITMAP *dib) {
	// XMP keyword
	char *g_png_xmp_keyword = "XML:com.adobe.xmp";
...
Load(FreeImageIO *io, FreeImageMem* mem, fi_handle handle, int page, int flags, void *data) {
	...
	if (handle) {
		...
		try {		
			...
			png_ptr = png_create_read_struct_2(PNG_LIBPNG_VER_STRING, (png_voidp)NULL, error_handler, warning_handler, mem->temp_mem,PngMemAlloc,PngMemFree);
			...
			switch (color_type) {
				case PNG_COLOR_TYPE_RGB:
					...
					if(image_type == FIT_BITMAP) {
						dib = FreeImage_AllocateHeader(mem, header_only, width, height, 24, FI_RGBA_RED_MASK, FI_RGBA_GREEN_MASK, FI_RGBA_BLUE_MASK);
					} else {
						dib = FreeImage_AllocateHeaderT(mem, header_only, image_type, width, height, pixel_depth);
					}
					break;

				case PNG_COLOR_TYPE_RGB_ALPHA:
					if(image_type == FIT_BITMAP) {
						dib = FreeImage_AllocateHeader(mem, header_only, width, height, 32, FI_RGBA_RED_MASK, FI_RGBA_GREEN_MASK, FI_RGBA_BLUE_MASK);
					} else {
						dib = FreeImage_AllocateHeaderT(mem, header_only, image_type, width, height, pixel_depth);
					}
					break;
				...
				case PNG_COLOR_TYPE_PALETTE:
					dib = FreeImage_AllocateHeader(mem, header_only, width, height, pixel_depth);
			...
			if (png_get_valid(png_ptr, info_ptr, PNG_INFO_iCCP)) {
				...
				FreeImage_CreateICCProfile(mem, dib, profile_data, profile_length);
			}
			...
			if (header_only) {
				...
				ReadMetadata(mem, png_ptr, info_ptr, dib);
				if (png_ptr) {
			...
			row_pointers = (png_bytepp)FreeImage_Malloc(mem->temp_mem,height * sizeof(png_bytep));
			...
			if (row_pointers) {
				FreeImage_Free(mem->temp_mem,row_pointers);
				row_pointers = NULL;
			}
...
Save(FreeImageIO *io, FreeImageMem* mem, FIBITMAP *dib, fi_handle handle, int page, int flags, void *data) {
	...
	if ((dib) && (handle)) {
		try {
			...
			WriteMetadata(mem, png_ptr, info_ptr, dib);
			...
			if ((pixel_depth == 32) && (!has_alpha_channel)) {
				BYTE *buffer = (BYTE *)FreeImage_Malloc(mem->temp_mem, width * 3);
				...
				FreeImage_Free(mem->temp_mem, buffer);
			} else {
				// the number of passes is either 1 for non-interlaced images, or 7 for interlaced images
				for (int pass = 0; pass < number_passes; pass++) {
					for (png_uint_32 k = 0; k < height; k++) {			
