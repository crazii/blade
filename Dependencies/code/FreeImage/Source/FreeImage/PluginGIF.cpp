...
static FreeImageMemType* s_temp_mem = NULL;
...
public:
	...
	{
		void* ptr = FreeImage_Malloc( s_temp_mem, n*sizeof(T) );
...
protected:
	...
	std::basic_string<char,std::char_traits<char>, TempAllocator<char> > m_strings[MAX_LZW_CODE]; //This is what is really the "string table" data for the Decompressor
	int* m_strmap;

	//input buffer
	BYTE *m_buffer;
	int m_bufferSize, m_bufferRealSize, m_bufferPos, m_bufferShift;
	FreeImageMem* memobject;
...
static BOOL 
FreeImage_SetMetadataEx(FREE_IMAGE_MDMODEL model, FreeImageMem* mem, FIBITMAP *dib, const char *key, WORD id, FREE_IMAGE_MDTYPE type, DWORD count, DWORD length, const void *value)
{
	BOOL bResult = FALSE;
	FITAG *tag = FreeImage_CreateTag(mem);
	if(tag) {
		FreeImage_SetTagKey(mem, tag, key);
		FreeImage_SetTagID(tag, id);
		FreeImage_SetTagType(tag, type);
		FreeImage_SetTagCount(tag, count);
		FreeImage_SetTagLength(tag, length);
		FreeImage_SetTagValue(mem, tag, value);
		if(model == FIMD_ANIMATION) {
			TagLib& s = TagLib::instance();
			// get the tag description
			const char *description = s.getTagDescription(TagLib::ANIMATION, id);
			FreeImage_SetTagDescription(mem, tag, description);
		}
		// store the tag
		bResult = FreeImage_SetMetadata(model, mem, dib, key, tag);
		FreeImage_DeleteTag(mem, tag);
	}
	return bResult;
}

static BOOL 
FreeImage_GetMetadataEx(FREE_IMAGE_MDMODEL model, FreeImageMem* mem, FIBITMAP *dib, const char *key, FREE_IMAGE_MDTYPE type, FITAG **tag)
{
...
StringTable::StringTable(FreeImageMem* mem)
{
	s_temp_mem = mem->temp_mem;
	memobject = mem;
	m_buffer = NULL;
	...
	m_strmap = (int*)FreeImage_Malloc(mem->temp_mem, sizeof(int)*(1<<20) );
}

StringTable::~StringTable()
{
	if( m_buffer != NULL ) {
		FreeImage_Free(memobject->temp_mem, m_buffer);
	}
	if( m_strmap != NULL ) {
		FreeImage_Free(memobject->temp_mem, m_strmap);
		m_strmap = NULL;
	}
}
...
{
	if( m_buffer == NULL ) {
		m_buffer = (BYTE*)FreeImage_Malloc(memobject->temp_mem, sizeof(BYTE)*len );
...
Open(FreeImageIO *io, FreeImageMem* mem, fi_handle handle, BOOL read) {
	GIFinfo *info = new( FreeImage_Malloc(mem->temp_mem,sizeof(GIFinfo)) ) GIFinfo;
	if( info == NULL ) {
		return NULL;
	}
	...
	if( read ) {
		try {
			...
			//delete info;
			info->~GIFinfo();
			FreeImage_Free(mem->temp_mem,info);
			return NULL;
		}
	} else {
		//Header
...
static void DLL_CALLCONV 
Close(FreeImageIO *io, FreeImageMem* mem, fi_handle handle, void *data) {
	if( data == NULL ) {
		return;
	}
...
Load(FreeImageIO *io, FreeImageMem* mem, fi_handle handle, int page, int flags, void *data) {
	if( data == NULL ) {
		return NULL;
	}
	GIFinfo *info = (GIFinfo *)data;
	...
	try {
		...
		if( (flags & GIF_PLAYBACK) == GIF_PLAYBACK ) {
			...
			dib = FreeImage_Allocate(mem, logicalwidth, logicalheight, 32);
			if( dib == NULL ) {
			...
			for( page = start; page <= end; page++ ) {
				...
				//decode page
				FIBITMAP *pagedib = Load(io, &tmem, handle, page, GIF_LOAD256, data);
				if( pagedib != NULL ) {
					...
					if( page == end ) {
						FITAG *tag;
						if( FreeImage_GetMetadataEx(FIMD_ANIMATION, mem, pagedib, "FrameTime", FIDT_LONG, &tag) ) {
							delay_time = *(LONG *)FreeImage_GetTagValue(tag);
						}
					}
					FreeImage_Unload(&tmem, pagedib);
				}
			}

			//setup frame time
			FreeImage_SetMetadataEx(FIMD_ANIMATION, mem, dib, "FrameTime", ANIMTAG_FRAMETIME, FIDT_LONG, 1, 4, &delay_time);
			return dib;
		}
		...
		dib = FreeImage_Allocate(mem, width, height, bpp);
		if( dib == NULL ) {
			throw FI_MSG_ERROR_DIB_MEMORY;
		}

		FreeImage_SetMetadataEx(FIMD_ANIMATION, mem, dib, "FrameLeft", ANIMTAG_FRAMELEFT, FIDT_SHORT, 1, 2, &left);
		FreeImage_SetMetadataEx(FIMD_ANIMATION, mem, dib, "FrameTop", ANIMTAG_FRAMETOP, FIDT_SHORT, 1, 2, &top);
		b = no_local_palette ? 1 : 0;
		FreeImage_SetMetadataEx(FIMD_ANIMATION, mem, dib, "NoLocalPalette", ANIMTAG_NOLOCALPALETTE, FIDT_BYTE, 1, 1, &b);
		b = interlaced ? 1 : 0;
		FreeImage_SetMetadataEx(FIMD_ANIMATION, mem, dib, "Interlaced", ANIMTAG_INTERLACED, FIDT_BYTE, 1, 1, &b);
		...
		StringTable *stringtable = new( FreeImage_Malloc(mem->temp_mem, sizeof(StringTable)) ) StringTable(mem);
		...
		if( page == 0 ) {
			...
			SwapShort(&logicalheight);
#endif
			FreeImage_SetMetadataEx(FIMD_ANIMATION, mem, dib, "LogicalWidth", ANIMTAG_LOGICALWIDTH, FIDT_SHORT, 1, 2, &logicalwidth);
			FreeImage_SetMetadataEx(FIMD_ANIMATION, mem, dib, "LogicalHeight", ANIMTAG_LOGICALHEIGHT, FIDT_SHORT, 1, 2, &logicalheight);
			...
			if( info->global_color_table_offset != 0 ) {
				...
				FreeImage_SetMetadataEx(FIMD_ANIMATION, mem, dib, "GlobalPalette", ANIMTAG_GLOBALPALETTE, FIDT_PALETTE, info->global_color_table_size, info->global_color_table_size * 4, globalpalette);
				//background color
			...
			FreeImage_SetMetadataEx(FIMD_ANIMATION, mem, dib, "Loop", ANIMTAG_LOOP, FIDT_LONG, 1, 4, &loop);
			...
			for( idx = 0; idx < info->comment_extension_offsets.size(); idx++ ) {
				...
				FreeImage_SetMetadataEx(FIMD_COMMENTS, mem, dib, buf, 1, FIDT_ASCII, comment_size, comment_size, comment.c_str());
			}
		}
		...
		FreeImage_SetMetadataEx(FIMD_ANIMATION, mem, dib, "DisposalMethod", ANIMTAG_DISPOSALMETHOD, FIDT_BYTE, 1, 1, &b);

		//delete stringtable;
		stringtable->~StringTable();
		FreeImage_Free(mem->temp_mem, stringtable);
...
static BOOL DLL_CALLCONV 
Save(FreeImageIO *io, FreeImageMem* mem, FIBITMAP *dib, fi_handle handle, int page, int flags, void *data) {
	if( data == NULL ) {
		return FALSE;
	}
	//GIFinfo *info = (GIFinfo *)data;
	...
	try {
		...
		if( FreeImage_GetMetadataEx(FIMD_ANIMATION, mem, dib, "FrameLeft", FIDT_SHORT, &tag) ) {
			left = *(WORD *)FreeImage_GetTagValue(tag);
		}
		if( FreeImage_GetMetadataEx(FIMD_ANIMATION, mem, dib, "FrameTop", FIDT_SHORT, &tag) ) {
			top = *(WORD *)FreeImage_GetTagValue(tag);
		}
		if( FreeImage_GetMetadataEx(FIMD_ANIMATION, mem, dib, "NoLocalPalette", FIDT_BYTE, &tag) ) {
			no_local_palette = *(BYTE *)FreeImage_GetTagValue(tag) ? true : false;
		}
		if( FreeImage_GetMetadataEx(FIMD_ANIMATION, mem, dib, "Interlaced", FIDT_BYTE, &tag) ) {
			interlaced = *(BYTE *)FreeImage_GetTagValue(tag) ? true : false;
		}
		if( FreeImage_GetMetadataEx(FIMD_ANIMATION, mem, dib, "FrameTime", FIDT_LONG, &tag) ) {
			delay_time = *(LONG *)FreeImage_GetTagValue(tag);
		}
		if( FreeImage_GetMetadataEx(FIMD_ANIMATION, mem, dib, "DisposalMethod", FIDT_BYTE, &tag) ) {
		...
		if( page == 0 ) {
			...
			if( FreeImage_GetMetadataEx(FIMD_ANIMATION, mem, dib, "LogicalWidth", FIDT_SHORT, &tag) ) {
			...
			if( FreeImage_GetMetadataEx(FIMD_ANIMATION, mem, dib, "LogicalHeight", FIDT_SHORT, &tag) ) {
			...
			if( FreeImage_GetMetadataEx(FIMD_ANIMATION, mem, dib, "GlobalPalette", FIDT_PALETTE, &tag) ) {
			...
			//Application Extension
			LONG loop = 0;
			if( FreeImage_GetMetadataEx(FIMD_ANIMATION, mem, dib, "Loop", FIDT_LONG, &tag) ) {
			...
			FITAG *tag = NULL;
			mdhandle = FreeImage_FindFirstMetadata(mem, FIMD_COMMENTS, dib, &tag);
			if( mdhandle ) {
				do {
				...
				FreeImage_FindCloseMetadata(mem, mdhandle);
			}
		}
