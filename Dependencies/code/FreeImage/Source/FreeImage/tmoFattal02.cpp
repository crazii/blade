...
@see GaussianPyramid
*/
static FIBITMAP* GaussianLevel5x5(FreeImageMem* mem, FIBITMAP *dib) {
	...
	try {
		...
		h_dib = FreeImage_AllocateT(&tmem,image_type, width, height);
		v_dib = FreeImage_AllocateT(&tmem,image_type, width, height);
		...
		FreeImage_Unload(&tmem, h_dib); h_dib = NULL;

		// perform downsampling

		dst = FreeImage_Rescale(mem, v_dib, width/2, height/2, FILTER_BILINEAR);

		FreeImage_Unload(&tmem, v_dib);

		return dst;

	} catch(int) {
		if(h_dib) FreeImage_Unload(&tmem, h_dib);
		if(v_dib) FreeImage_Unload(&tmem, v_dib);
...
*/
static BOOL GaussianPyramid(FreeImageMem* mem, FIBITMAP *H, FIBITMAP **pyramid, int nlevels) {
	try {
		// first level is the original image
		pyramid[0] = FreeImage_Clone(mem, H);
		if(pyramid[0] == NULL) throw(1);
		// compute next levels
		for(int k = 1; k < nlevels; k++) {
			pyramid[k] = GaussianLevel5x5(mem, pyramid[k-1]);
		...
		for(int k = 0; k < nlevels; k++) {
			if(pyramid[k] != NULL) {
				FreeImage_Unload(mem, pyramid[k]);
				pyramid[k] = NULL;
			}
		}
		return FALSE;
...
@see GradientPyramid
*/
static FIBITMAP* GradientLevel(FreeImageMem* mem, FIBITMAP *H, float *avgGrad, int k) {
	FIBITMAP *G = NULL;
	...
	try {
		...
		G = FreeImage_AllocateT(mem, image_type, width, height);
		if(!G) throw(1);
...
@return Returns TRUE if successful, returns FALSE otherwise
*/
static BOOL GradientPyramid(FreeImageMem* mem, FIBITMAP **pyramid, int nlevels, FIBITMAP **gradients, float *avgGrad) {
	try {
		for(int k = 0; k < nlevels; k++) {
			FIBITMAP *Hk = pyramid[k];
			gradients[k] = GradientLevel(mem, Hk, &avgGrad[k], k);
		...
		for(int k = 0; k < nlevels; k++) {
			if(gradients[k] != NULL) {
				FreeImage_Unload(mem, gradients[k]);
				gradients[k] = NULL;
			}
		}
		return FALSE;
...
*/
static FIBITMAP* PhiMatrix(FreeImageMem* mem, FIBITMAP **gradients, float *avgGrad, int nlevels, float alpha, float beta) {
	float *src_pixel, *dst_pixel;
	FIBITMAP **phi = NULL;
	...
	try {
		phi = (FIBITMAP**)FreeImage_Malloc(mem->temp_mem,nlevels * sizeof(FIBITMAP*));
		if(!phi) throw(1);
		...
		for(int k = nlevels-1; k >= 0; k--) {
			...
			if( k == 0 )
				phi[k] = FreeImage_AllocateT(mem, FIT_FLOAT, width, height);
			else
				phi[k] = FreeImage_AllocateT(&tmem, FIT_FLOAT, width, height);
			if(!phi[k]) throw(1);
			...
			if(k < nlevels-1) {
				...
				FIBITMAP *L = FreeImage_Rescale(&tmem, phi[k+1], width, height, FILTER_BILINEAR);
				if(!L) throw(1);
				...
				FreeImage_Unload(&tmem, L);

				// PHI(k+1) is no longer needed
				FreeImage_Unload(&tmem, phi[k+1]);
				phi[k+1] = NULL;
			}
		...
		FreeImage_Free(mem->temp_mem,phi);
		...
		if(phi) {
			for(int k = nlevels-1; k >= 1; k--) {
				if(phi[k]) FreeImage_Unload(&tmem, phi[k]);
			if( phi[0])	FreeImage_Unload(mem,phi[0]);
			}
			FreeImage_Free(mem->temp_mem,phi);
		}
		return NULL;
...
*/
static FIBITMAP* Divergence(FreeImageMem* mem, FIBITMAP *H, FIBITMAP *PHI) {
	...
	try {
		...
		Gx = FreeImage_AllocateT(&tmem,image_type, width, height);
		if(!Gx) throw(1);
		Gy = FreeImage_AllocateT(&tmem,image_type, width, height);
		if(!Gy) throw(1);
		...
		divG = FreeImage_AllocateT(mem, image_type, width, height);
		if(!divG) throw(1);
		...
		// no longer needed ... 
		FreeImage_Unload(&tmem, Gx);
		FreeImage_Unload(&tmem, Gy);

		// return the divergence
		return divG;

	} catch(int) {
		if(Gx) FreeImage_Unload(&tmem, Gx);
		if(Gy) FreeImage_Unload(&tmem, Gy);
		if(divG) FreeImage_Unload(mem, divG);
		return NULL;
	}
}
...
*/
static FIBITMAP* tmoFattal02(FreeImageMem* mem, FIBITMAP *Y, float alpha, float beta) {
	...
	try {
		...
		FIBITMAP *H = LogLuminance(&tmem, Y);
		if(!H) throw(1);
		...
		pyramid = (FIBITMAP**)FreeImage_Malloc(mem->temp_mem,nlevels * sizeof(FIBITMAP*));
		if(!pyramid) throw(1);
		memset(pyramid, 0, nlevels * sizeof(FIBITMAP*));

		if(!GaussianPyramid(&tmem,H, pyramid, nlevels)) throw(1);

		// calculate gradient magnitude and its average value on each pyramid level
		gradients = (FIBITMAP**)FreeImage_Malloc(mem->temp_mem,nlevels * sizeof(FIBITMAP*));
		if(!gradients) throw(1);
		memset(gradients, 0, nlevels * sizeof(FIBITMAP*));
		avgGrad = (float*)FreeImage_Malloc(mem->temp_mem,nlevels * sizeof(float));
		if(!avgGrad) throw(1);

		if(!GradientPyramid(&tmem, pyramid, nlevels, gradients, avgGrad)) throw(1);

		// free the Gaussian pyramid
		for(k = 0; k < nlevels; k++) {
			if(pyramid[k]) FreeImage_Unload(&tmem, pyramid[k]);
		}
		FreeImage_Free(mem->temp_mem,pyramid); pyramid = NULL;

		// compute the gradient attenuation function PHI(x, y)
		phy = PhiMatrix(&tmem, gradients, avgGrad, nlevels, alpha, beta);
		if(!phy) throw(1);

		// free the gradient pyramid
		for(k = 0; k < nlevels; k++) {
			if(gradients[k]) FreeImage_Unload(&tmem, gradients[k]);
		}
		FreeImage_Free(mem->temp_mem,gradients); gradients = NULL;
		FreeImage_Free(mem->temp_mem,avgGrad); avgGrad = NULL;

		// compute gradients in x and y directions, attenuate them with the attenuation matrix, 
		// then compute the divergence div G from the attenuated gradient. 
		divG = Divergence(&tmem, H, phy);
		if(!divG) throw(1);

		// H & phy no longer needed
		FreeImage_Unload(&tmem, H); H = NULL;
		FreeImage_Unload(&tmem, phy); phy = NULL;

		// solve the PDE (Poisson equation) using a multigrid solver and 3 cycles
		FIBITMAP *U = FreeImage_MultigridPoissonSolver(mem, divG, 3);
		if(!U) throw(1);

		FreeImage_Unload(mem, divG);
		...
		if(H) FreeImage_Unload(&tmem, H);
		if(pyramid) {
			for(int i = 0; i < nlevels; i++) {
				if(pyramid[i]) FreeImage_Unload(&tmem, pyramid[i]);
			}
			free(pyramid);
		}
		if(gradients) {
			for(int i = 0; i < nlevels; i++) {
				if(gradients[i]) FreeImage_Unload(&tmem, gradients[i]);
			}
			free(gradients);
		}
		if(avgGrad) FreeImage_Free(mem->temp_mem,avgGrad);
		if(phy) FreeImage_Unload(&tmem, phy);
		if(divG) FreeImage_Unload(&tmem, divG);
		if(U) FreeImage_Unload(mem, U);
...
FreeImage_TmoFattal02(FreeImageMem* mem, FIBITMAP *dib, double color_saturation, double attenuation) {	
	...
	try {
		...
		// convert to RGBF
		src = FreeImage_ConvertToRGBF(&tmem, dib);
		if(!src) throw(1);

		// get the luminance channel
		Yin = ConvertRGBFToY(&tmem, src);
		if(!Yin) throw(1);

		// perform the tone mapping
		Yout = tmoFattal02(&tmem, Yin, alpha, beta);
		if(!Yout) throw(1);
		...
		// not needed anymore
		FreeImage_Unload(&tmem, Yin);  Yin  = NULL;
		FreeImage_Unload(&tmem, Yout); Yout = NULL;

		// clamp image highest values to display white, then convert to 24-bit RGB
		dst = ClampConvertRGBFTo24(mem, src);

		// clean-up and return
		FreeImage_Unload(&tmem, src); src = NULL;

		// copy metadata from src to dst
		FreeImage_CloneMetadata(mem, dst, dib);
		
		return dst;

	} catch(int) {
		if(src) FreeImage_Unload(&tmem, src);
		if(Yin) FreeImage_Unload(&tmem, Yin);
		if(Yout) FreeImage_Unload(&tmem, Yout);
		return NULL;
	}
}
