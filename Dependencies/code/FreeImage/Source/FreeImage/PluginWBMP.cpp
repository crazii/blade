...
static void
readExtHeader(FreeImageIO *io, FreeImageMem* mem, fi_handle handle, BYTE b) {
	...
	switch (b & 0x60) {
		...
		case 0x60:
		{
			...
			BYTE *Ident = (BYTE*)FreeImage_Malloc(mem->temp_mem, sizeParamIdent * sizeof(BYTE));
			BYTE *Value = (BYTE*)FreeImage_Malloc(mem->temp_mem, sizeParamValue * sizeof(BYTE));
		
			io->read_proc(Ident, sizeParamIdent, 1, handle);
			io->read_proc(Value, sizeParamValue, 1, handle);
			
			FreeImage_Free(mem->temp_mem, Ident);
			FreeImage_Free(mem->temp_mem, Value);
			break;
		}		
		...
		try {
			...
			if (header.FixHeaderField & 0x80) {
				...
				while(header.ExtHeaderFields & 0x80) {
					...
					readExtHeader(io, mem, handle, header.ExtHeaderFields);
				}
			}
			...
			dib = FreeImage_Allocate(mem, width, height, 1);
			if (!dib) {
