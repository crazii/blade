...
FreeImage_ConvertTo16Bits555(FreeImageMem* mem, FIBITMAP *dib) {
	...
	if(bpp == 16) {
		if ((FreeImage_GetRedMask(dib) == FI16_565_RED_MASK) && (FreeImage_GetGreenMask(dib) == FI16_565_GREEN_MASK) && (FreeImage_GetBlueMask(dib) == FI16_565_BLUE_MASK)) {
			// RGB 565
			FIBITMAP *new_dib = FreeImage_Allocate(mem, width, height, 16, FI16_555_RED_MASK, FI16_555_GREEN_MASK, FI16_555_BLUE_MASK);
			if(new_dib == NULL) {
				return NULL;
			}
