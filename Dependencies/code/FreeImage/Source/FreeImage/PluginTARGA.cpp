...
{
public:
	TargaThumbnail(FreeImageMem* mem) : _w(0), _h(0), _depth(0), _data(NULL) ,_mem(mem){ 
	}
	~TargaThumbnail() { 
		if(_data) {
			//free(_data); 
			FreeImage_AlignFree(_mem->temp_mem, _data);
		}
	}
	...
	BOOL read(FreeImageIO *io, fi_handle handle, size_t size) {
		...
		_data = (BYTE*)FreeImage_Malloc(_mem->temp_mem, sizeofData);
		if(_data) {
...
FIBITMAP* TargaThumbnail::toFIBITMAP() {
	...
	FIBITMAP* dib = FreeImage_Allocate(_mem,_w, _h, _depth);
	if(!dib) {
		return NULL;
	}
...
{
public:
	IOCache(FreeImageMem* mem,FreeImageIO *io, fi_handle handle, size_t size) :
		_ptr(NULL), _begin(NULL), _end(NULL), _size(size), _io(io), _handle(handle)	{
			_mem = mem;
			_begin = (BYTE*)FreeImage_Malloc(_mem->temp_mem,size);
			if (_begin) {
			_end = _begin + _size;
	...
	~IOCache() {
		if (_begin != NULL) {
			FreeImage_Free(_mem->temp_mem,_begin);
		}	
	}
...
*/
static void 
loadTrueColor(FreeImageMem* mem,FIBITMAP* dib, int width, int height, int file_pixel_size, FreeImageIO* io, fi_handle handle, BOOL as24bit) {
	const int pixel_size = as24bit ? 3 : file_pixel_size;

	// input line cache
	BYTE* file_line = (BYTE*)FreeImage_Malloc(mem->temp_mem, width * file_pixel_size);
	...
	FreeImage_Free(mem->temp_mem,file_line);
}
...
template<int bPP>
static void 
loadRLE(FreeImageMem* mem,FIBITMAP* dib, int width, int height, FreeImageIO* io, fi_handle handle, long eof, BOOL as24bit) {
	...
	IOCache cache(mem,io, handle, sz);
	if(cache.isNull()) {
...
Load(FreeImageIO *io,FreeImageMem* mem, fi_handle handle, int page, int flags, void *data) {
	...
	try {
		...
		switch (header.is_pixel_depth) {
			case 8 : {
				dib = FreeImage_AllocateHeader(mem, header_only, header.is_width, header.is_height, 8);
				...
				if (header.color_map_type > 0) {
					...
					BYTE *cmap = (BYTE*)FreeImage_Malloc(mem->temp_mem,csize * sizeof(BYTE));
					...
					FreeImage_Free(mem->temp_mem,cmap);
				}
				...
				if(th) {
					...
					FreeImage_SetThumbnail(mem, dib, th);
					FreeImage_Unload(&tmem,th);				
				}
				...
				switch (header.image_type) {
					...
					case TGA_RLEMONO: { //(8 bit)
						loadRLE<8>(mem,dib, header.is_width, header.is_height, io, handle, eof, FALSE);
					}
					break;
			...
			case 16 : {
				...
				if (TARGA_LOAD_RGB888 & flags) {
					pixel_bits = 24;
					dib = FreeImage_AllocateHeader(mem, header_only, header.is_width, header.is_height, pixel_bits, FI_RGBA_RED_MASK, FI_RGBA_GREEN_MASK, FI_RGBA_BLUE_MASK);

				} else {
					dib = FreeImage_AllocateHeader(mem, header_only, header.is_width, header.is_height, pixel_bits, FI16_555_RED_MASK, FI16_555_GREEN_MASK, FI16_555_BLUE_MASK);
				}
				...
				if(th) {
					if(TARGA_LOAD_RGB888 & flags) {
						FIBITMAP* t = FreeImage_ConvertTo24Bits(&tmem, th);
						FreeImage_Unload(&tmem,th);
						th = t;
					}
				...
				switch (header.image_type) {
					case TGA_RGB: { //(16 bit)
						// input line cache
						BYTE *in_line = (BYTE*)FreeImage_Malloc(mem->temp_mem,header.is_width * sizeof(WORD));
						...
						FreeImage_Free(mem->temp_mem,in_line);
					}
					break;

					case TGA_RLERGB: { //(16 bit)
						loadRLE<16>(mem,dib, header.is_width, header.is_height, io, handle, eof, TARGA_LOAD_RGB888 & flags);
					}
					break;
			...
			case 24 : {
				...
				switch (header.image_type) {
					case TGA_RGB: { //(24 bit)
						//uncompressed
						loadTrueColor(mem,dib, header.is_width, header.is_height, pixel_size,io, handle, TRUE);
					}
					break;

					case TGA_RLERGB: { //(24 bit)
						loadRLE<24>(mem,dib, header.is_width, header.is_height, io, handle, eof, TRUE);
					}
					break;
			...
			case 32 : {
				...
				switch (header.image_type) {
					case TGA_RGB: { //(32 bit)
						// uncompressed
						loadTrueColor(mem,dib, header.is_width, header.is_height, 4 /*file_pixel_size*/, io, handle, TARGA_LOAD_RGB888 & flags);
					}
					break;

					case TGA_RLERGB: { //(32 bit)
						loadRLE<32>(mem,dib, header.is_width, header.is_height, io, handle, eof, TARGA_LOAD_RGB888 & flags);
					}
					break;
		...
		if (flipvert) {
			FreeImage_FlipVertical(mem, dib);
		}

		if (fliphoriz) {
			FreeImage_FlipHorizontal(mem, dib);
		}
...
static void 
saveRLE(FreeImageMem* mem,FIBITMAP* dib, FreeImageIO* io, fi_handle handle) {
	...
	BYTE* const packet_begin = (BYTE*)FreeImage_Malloc(mem->temp_mem,max_packet_size * pixel_size);
	...
	BYTE* const line_begin = (BYTE*)FreeImage_Malloc(mem->temp_mem,width * pixel_size + extra_space);
	BYTE* line = line_begin;

	BYTE *current = (BYTE*)FreeImage_Malloc(mem->temp_mem,pixel_size);
	BYTE *next    = (BYTE*)FreeImage_Malloc(mem->temp_mem,pixel_size);
	...
	FreeImage_Free(mem->temp_mem,line_begin);
	FreeImage_Free(mem->temp_mem,packet_begin);
	FreeImage_Free(mem->temp_mem,current);
	FreeImage_Free(mem->temp_mem,next);
}
...
Save(FreeImageIO *io, FreeImageMem* mem, FIBITMAP *dib, fi_handle handle, int page, int flags, void *data) {
	...
	if (palette) {
		if (FreeImage_IsTransparent(dib)) {
			FILE_BGRA *bgra_pal = (FILE_BGRA*)FreeImage_Malloc(mem->temp_mem,header.cm_length * sizeof(FILE_BGRA));
			...
			FreeImage_Free(mem->temp_mem,bgra_pal);

		} else {
			FILE_BGR *bgr_pal = (FILE_BGR*)FreeImage_Malloc(mem->temp_mem,header.cm_length * sizeof(FILE_BGR));
			...
			FreeImage_Free(mem->temp_mem,bgr_pal);
		}
	}
	...
	if (TARGA_SAVE_RLE & flags) {
		...
		BYTE *line, *const line_begin = (BYTE*)FreeImage_Malloc(mem->temp_mem,width * pixel_size);
