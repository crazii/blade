...
Open(FreeImageIO *io, FreeImageMem* mem, fi_handle handle, BOOL read) {
	...
	ICONHEADER *lpIH = (ICONHEADER*)FreeImage_Malloc(mem->normal_mem, sizeof(ICONHEADER));
	if(lpIH == NULL) {
		return NULL;
	}
	...
	if (read) {
		...
		if(!(lpIH->idReserved == 0) || !(lpIH->idType == 1)) {
			// Not an ICO file
			FreeImage_Free(mem->normal_mem, lpIH);
			return NULL;
		}
	}
	else {
...
Load(FreeImageIO *io, FreeImageMem* mem, fi_handle handle, int page, int flags, void *data) {
	...
	if (handle != NULL) {
		...
		if (icon_header) {
			...
			ICONDIRENTRY *icon_list = (ICONDIRENTRY*)FreeImage_Malloc(mem->temp_mem,icon_header->idCount * sizeof(ICONDIRENTRY));
			if(icon_list == NULL) {
				return NULL;
			}
			...
			if (page < icon_header->idCount) {
				...
				if((icon_list[page].bWidth == 0) && (icon_list[page].bHeight == 0)) {
					dib = FreeImage_LoadFromHandle(FIF_PNG, io, mem, handle, header_only ? FIF_LOAD_NOPIXELS : PNG_DEFAULT);
					FreeImage_Free(mem->temp_mem,icon_list);
					return dib;
				}
				...
				if((flags & ICO_MAKEALPHA) == ICO_MAKEALPHA) {
					FIBITMAP *dib32 = FreeImage_ConvertTo32Bits(mem, dib);
					...
					BYTE *line_and	= (BYTE *)FreeImage_Malloc(mem->temp_mem,width_and);

					if( line_and == NULL ) {
						FreeImage_Unload(mem, dib32);
						return NULL;
					}
					...
					FreeImage_Free(mem->temp_mem,line_and);
...
Save(FreeImageIO *io, FreeImageMem* mem, FIBITMAP *dib, fi_handle handle, int page, int flags, void *data) {
	...
	if(icon_header) {
		...
		for(k = 0; k < icon_header->idCount; k++) {
			icon_dib = Load(io, &tmem, handle, k, flags, data);
			vPages.push_back(icon_dib);
		}

		// add the page
		icon_dib = FreeImage_Clone(&tmem, dib);
		vPages.push_back(icon_dib);
		icon_header->idCount++;
		...
		for(k = 0; k < icon_header->idCount; k++) {
...
#endif
			// AND mask
			BYTE *and_mask = (BYTE*)FreeImage_Malloc(mem->temp_mem,size_and);
			...
			FreeImage_Free(mem->temp_mem,and_mask);
		...
		for(k = 0; k < icon_header->idCount; k++) {
			icon_dib = (FIBITMAP*)vPages[k];
			FreeImage_Unload(&tmem,icon_dib);
		}
