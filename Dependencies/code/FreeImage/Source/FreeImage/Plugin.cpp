...
void * DLL_CALLCONV
FreeImage_Open(PluginNode *node, FreeImageIO *io, FreeImageMem* mem, fi_handle handle, BOOL open_for_reading) {
	if (node->m_plugin->open_proc != NULL) {
       return node->m_plugin->open_proc(io, mem, handle, open_for_reading);
	}
...
void DLL_CALLCONV
FreeImage_Close(PluginNode *node, FreeImageIO *io, FreeImageMem* mem, fi_handle handle, void *data) {
	if (node->m_plugin->close_proc != NULL) {
		node->m_plugin->close_proc(io, mem, handle, data);
	}
}
...
FreeImage_LoadFromHandle(FREE_IMAGE_FORMAT fif, FreeImageIO *io, FreeImageMem* mem, fi_handle handle, int flags) {
		...
		if (node != NULL) {
			if (node->m_enabled) {
				if(node->m_plugin->load_proc != NULL) {
					...
					void *data = FreeImage_Open(node, io, mem, handle, TRUE);
					
					bitmap = node->m_plugin->load_proc(io, mem, handle, -1, flags, data);
...
FIBITMAP * DLL_CALLCONV
FreeImage_Load(FREE_IMAGE_FORMAT fif, FreeImageMem* mem, const char *filename, int flags) {
	FreeImageIO io;
	SetDefaultIO(&io);
	...
	if (handle) {
		FIBITMAP *bitmap = FreeImage_LoadFromHandle(fif, &io, mem, (fi_handle)handle, flags);
...
FIBITMAP * DLL_CALLCONV
FreeImage_LoadU(FREE_IMAGE_FORMAT fif, FreeImageMem* mem, const wchar_t *filename, int flags) {
	FreeImageIO io;
	SetDefaultIO(&io);
#ifdef _WIN32	
...
BOOL DLL_CALLCONV
FreeImage_SaveToHandle(FREE_IMAGE_FORMAT fif, FIBITMAP *dib, FreeImageIO *io, FreeImageMem* mem, fi_handle handle, int flags) {
		...
		if (node) {
			if (node->m_enabled) {
				if(node->m_plugin->save_proc != NULL) {
					...
					result = node->m_plugin->save_proc(io, mem, dib, handle, -1, flags, data);
...
BOOL DLL_CALLCONV
FreeImage_Save(FREE_IMAGE_FORMAT fif, FreeImageMem* mem, FIBITMAP *dib, const char *filename, int flags) {
	FreeImageIO io;
	SetDefaultIO(&io);
	...
	if (handle) {
		BOOL success = FreeImage_SaveToHandle(fif, dib, &io, mem, (fi_handle)handle, flags);
...
BOOL DLL_CALLCONV
FreeImage_SaveU(FREE_IMAGE_FORMAT fif, FreeImageMem* mem, FIBITMAP *dib, const wchar_t *filename, int flags) {
	FreeImageIO io;
	SetDefaultIO(&io);
#ifdef _WIN32	
...
FREE_IMAGE_FORMAT DLL_CALLCONV
FreeImage_GetFIFFromFilename(FreeImageMem* mem, const char *filename) {
	if (filename != NULL) {
		...
		for (int i = 0; i < FreeImage_GetFIFCount(); ++i) {
			...
			if (s_plugins->FindNodeFromFIF(i)->m_enabled) {
				...
				if (FreeImage_stricmp(FreeImage_GetFormatFromFIF((FREE_IMAGE_FORMAT)i), extension) == 0) {
					...
					char *copy = (char *)FreeImage_Malloc( mem->temp_mem, strlen(FreeImage_GetFIFExtensionList((FREE_IMAGE_FORMAT)i)) + 1);
					...
					while (token != NULL) {
						if (FreeImage_stricmp(token, extension) == 0) {
							FreeImage_Free(mem->temp_mem, copy);
...
FREE_IMAGE_FORMAT DLL_CALLCONV 
FreeImage_GetFIFFromFilenameU(FreeImageMem* mem, const wchar_t *filename) {
#ifdef _WIN32	
	...
	char *extension = (char *)FreeImage_Malloc(mem->temp_mem, wcslen(place)+1);
	unsigned int i=0;
	for(; i < wcslen(place); i++) // convert 16-bit to 8-bit
		extension[i] = (char)(place[i] & 0x00FF);
	// set terminating 0
	extension[i]=0;
	FREE_IMAGE_FORMAT fRet = FreeImage_GetFIFFromFilename(mem, extension);
	FreeImage_Free(mem->temp_mem, extension);
