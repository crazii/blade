...
FIBITMAP* 
RemoveAlphaChannel(FreeImageMem* mem, FIBITMAP* src) { 
	...
	switch(image_type) {
		case FIT_BITMAP:
			if(FreeImage_GetBPP(src) == 32) {
				// convert to 24-bit
				return FreeImage_ConvertTo24Bits(mem, src);
			}
			break;
		case FIT_RGBA16:
			// convert to RGB16
			return FreeImage_ConvertToRGB16(mem, src);
		case FIT_RGBAF:
			// convert to RGBF
			return FreeImage_ConvertToRGBF(mem, src);
		default:
...
FreeImage_ColorQuantize(FreeImageMem* mem, FIBITMAP *dib, FREE_IMAGE_QUANTIZE quantize) {
	return FreeImage_ColorQuantizeEx(mem, dib, quantize);
}

FIBITMAP * DLL_CALLCONV
FreeImage_ColorQuantizeEx(FreeImageMem* mem, FIBITMAP *dib, FREE_IMAGE_QUANTIZE quantize, int PaletteSize, int ReserveSize, RGBQUAD *ReservePalette) {
	FreeImageMem tmem;
	...
	if (FreeImage_HasPixels(dib)) {
		if (FreeImage_GetBPP(dib) == 24) {
			switch(quantize) {
				...
				{
					try {
						WuQuantizer Q (&tmem, dib);
						FIBITMAP *dst = Q.Quantize(mem, PaletteSize, ReserveSize, ReservePalette);
						if(dst) {
				...
				case FIQ_NNQUANT :
				{
					...
					NNQuantizer Q(&tmem, PaletteSize);
					FIBITMAP *dst = Q.Quantize(mem, dib, ReserveSize, ReservePalette, sampling);
					if(dst) {
						// copy metadata from src to dst
						FreeImage_CloneMetadata(mem, dst, dib);
					}
...
FIBITMAP * DLL_CALLCONV
FreeImage_ConvertFromRawBits(FreeImageMem* mem, BYTE *bits, int width, int height, int pitch, unsigned bpp, unsigned red_mask, unsigned green_mask, unsigned blue_mask, BOOL topdown) {
	FIBITMAP *dib = FreeImage_Allocate(mem, width, height, bpp, red_mask, green_mask, blue_mask);
