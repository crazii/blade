...
*/
static const char* 
readXBMFile(FreeImageIO *io, FreeImageMem* mem, fi_handle handle, int *widthP, int *heightP, char **dataP) {
	...
	*dataP = (char*) FreeImage_Malloc(mem->temp_mem, raster_length );
	if ( *dataP == (char*) 0 )
...
Load(FreeImageIO *io, FreeImageMem* mem, fi_handle handle, int page, int flags, void *data) {
	...
	try {
		...
		const char* error = readXBMFile(io, mem, handle, &width, &height, &buffer);
		...
		if(buffer)	FreeImage_Free(mem->temp_mem,buffer);
		if(dib)		FreeImage_Unload(mem, dib);
