...
*/
static BOOL 
LoadPixelDataRLE4(FreeImageIO *io, FreeImageMem* mem, fi_handle handle, int width, int height, FIBITMAP *dib) {
	int status_byte = 0;
	BYTE second_byte = 0;
	int bits = 0;
	...
	try {
		...
		pixels = (BYTE*)FreeImage_Malloc( mem->temp_mem, width * height * sizeof(BYTE));
		if(!pixels) throw(1);
		...
		FreeImage_Free(mem->temp_mem, pixels);

		return TRUE;

	} catch(int) {
		if(pixels) FreeImage_Free(mem->temp_mem, pixels);
		return FALSE;
	}
}
...
static FIBITMAP *
LoadWindowsBMP(FreeImageIO *io, FreeImageMem* mem, fi_handle handle, int flags, unsigned bitmap_bits_offset) {
	FIBITMAP *dib = NULL;
	...
	try {
		...
		switch (bit_count) {
			...
			case 8 :
			{
				...
				dib = FreeImage_AllocateHeader(mem, header_only, width, height, bit_count);
				if (dib == NULL) {
				...
				switch (compression) {
					...
					case BI_RLE4 :
						if( LoadPixelDataRLE4(io, mem, handle, width, height, dib) ) {
							return dib;
						} else {
			...
			case 16 :
			{
				if (bih.biCompression == BI_BITFIELDS) {
					...
					dib = FreeImage_AllocateHeader(mem, header_only, width, height, bit_count, bitfields[0], bitfields[1], bitfields[2]);
				} else {
					dib = FreeImage_AllocateHeader(mem, header_only, width, height, bit_count, FI16_555_RED_MASK, FI16_555_GREEN_MASK, FI16_555_BLUE_MASK);
				}
			...
			case 32 :
			{
				if (bih.biCompression == BI_BITFIELDS) {
					...
					if( bit_count == 32 ) {
						dib = FreeImage_AllocateHeader(mem, header_only, width, height, bit_count, FI_RGBA_RED_MASK, FI_RGBA_GREEN_MASK, FI_RGBA_BLUE_MASK);
					} else {
						dib = FreeImage_AllocateHeader(mem, header_only, width, height, bit_count, FI_RGBA_RED_MASK, FI_RGBA_GREEN_MASK, FI_RGBA_BLUE_MASK);
					}
				}
...
static FIBITMAP *
LoadOS22XBMP(FreeImageIO *io, FreeImageMem* mem, fi_handle handle, int flags, unsigned bitmap_bits_offset) {
	FIBITMAP *dib = NULL;
...
static FIBITMAP *
LoadOS21XBMP(FreeImageIO *io, FreeImageMem* mem, fi_handle handle, int flags, unsigned bitmap_bits_offset) {
	FIBITMAP *dib = NULL;
	...
	if (handle != NULL) {
		...
		switch(type) {
			case 12:
				...
				return LoadOS21XBMP(io, mem, handle, flags, offset_in_file + bitmapfileheader.bfOffBits);
			case 64:
				// OS/2
				return LoadOS22XBMP(io, mem, handle, flags, offset_in_file + bitmapfileheader.bfOffBits);
			case 40:
				// BITMAPINFOHEADER - all Windows versions since Windows 3.0
				return LoadWindowsBMP(io, mem, handle, flags, offset_in_file + bitmapfileheader.bfOffBits);
			case 52:
...
Save(FreeImageIO *io, FreeImageMem* mem, FIBITMAP *dib, fi_handle handle, int page, int flags, void *data) {
	if ((dib != NULL) && (handle != NULL)) {
		...
		if ((bpp == 8) && (flags & BMP_SAVE_RLE)) {
			BYTE *buffer = (BYTE*)FreeImage_Malloc( mem->temp_mem, FreeImage_GetPitch(dib) * 2 * sizeof(BYTE));
