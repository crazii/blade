...
{
public:
	FIBITMAP* convert(FreeImageMem* mem, FIBITMAP *src, FREE_IMAGE_TYPE dst_type);
};

template<class Tdst, class Tsrc> FIBITMAP* 
CONVERT_TYPE<Tdst, Tsrc>::convert(FreeImageMem* mem, FIBITMAP *src, FREE_IMAGE_TYPE dst_type) {
	...
	dst = FreeImage_AllocateT(mem, dst_type, width, height, bpp, 
...
{
public:
	FIBITMAP* convert(FreeImageMem* mem, FIBITMAP *src, BOOL scale_linear);
};

template<class Tsrc> FIBITMAP* 
CONVERT_TO_BYTE<Tsrc>::convert(FreeImageMem* mem, FIBITMAP *src, BOOL scale_linear) {
	FIBITMAP *dst = NULL;
	unsigned x, y;
	...
	dst = FreeImage_AllocateT(mem, FIT_BITMAP, width, height, 8, 0, 0, 0);
	if(!dst) return NULL;
...
{
public:
	FIBITMAP* convert(FreeImageMem* mem, FIBITMAP *src);
};

template<class Tsrc> FIBITMAP* 
CONVERT_TO_COMPLEX<Tsrc>::convert(FreeImageMem* mem, FIBITMAP *src) {
	FIBITMAP *dst = NULL;
	...
	dst = FreeImage_AllocateT(mem, FIT_COMPLEX, width, height);
	if(!dst) return NULL;
...
FreeImage_ConvertToStandardType(FreeImageMem* mem, FIBITMAP *src, BOOL scale_linear) {
	FIBITMAP *dst = NULL;
	...
	switch(src_type) {
		case FIT_BITMAP:	// standard image: 1-, 4-, 8-, 16-, 24-, 32-bit
			dst = FreeImage_Clone(mem, src);
			break;
		case FIT_UINT16:	// array of unsigned short: unsigned 16-bit
			dst = convertUShortToByte.convert(mem, src, scale_linear);
			break;
		case FIT_INT16:		// array of short: signed 16-bit
			dst = convertShortToByte.convert(mem, src, scale_linear);
			break;
		case FIT_UINT32:	// array of unsigned long: unsigned 32-bit
			dst = convertULongToByte.convert(mem, src, scale_linear);
			break;
		case FIT_INT32:		// array of long: signed 32-bit
			dst = convertLongToByte.convert(mem, src, scale_linear);
			break;
		case FIT_FLOAT:		// array of float: 32-bit
			dst = convertFloatToByte.convert(mem, src, scale_linear);
			break;
		case FIT_DOUBLE:	// array of double: 64-bit
			dst = convertDoubleToByte.convert(mem, src, scale_linear);
			break;
		case FIT_COMPLEX:	// array of FICOMPLEX: 2 x 64-bit
			{
				...
				FIBITMAP *dib_double = FreeImage_GetComplexChannel(&tmem, src, FICC_MAG);
				if(dib_double) {
					// Convert to a standard bitmap (linear scaling)
					dst = convertDoubleToByte.convert(mem, dib_double, scale_linear);
					// Free image of type FIT_DOUBLE
					FreeImage_Unload(&tmem, dib_double);
				}
			}
			break;
...
FIBITMAP* DLL_CALLCONV
FreeImage_ConvertToType(FreeImageMem* mem, FIBITMAP *src, FREE_IMAGE_TYPE dst_type, BOOL scale_linear) {
	FIBITMAP *dst = NULL;
	...
	if(src_type == dst_type) {
		return FreeImage_Clone(mem, src);
	}
	...
	switch(src_type) {
		case FIT_BITMAP:
			switch(dst_type) {
				case FIT_UINT16:
					dst = FreeImage_ConvertToUINT16(mem, src);
					break;
				case FIT_INT16:
					dst = (src_bpp == 8) ? convertByteToShort.convert(mem, src, dst_type) : NULL;
					break;
				case FIT_UINT32:
					dst = (src_bpp == 8) ? convertByteToULong.convert(mem, src, dst_type) : NULL;
					break;
				case FIT_INT32:
					dst = (src_bpp == 8) ? convertByteToLong.convert(mem, src, dst_type) : NULL;
					break;
				case FIT_FLOAT:
					dst = FreeImage_ConvertToFloat(mem, src);
					break;
				case FIT_DOUBLE:
					dst = (src_bpp == 8) ? convertByteToDouble.convert(mem, src, dst_type) : NULL;
					break;
				case FIT_COMPLEX:
					dst = (src_bpp == 8) ? convertByteToComplex.convert(mem, src) : NULL;
					break;
				case FIT_RGB16:
					dst = FreeImage_ConvertToRGB16(mem, src);
					break;
				case FIT_RGBA16:
					break;
				case FIT_RGBF:
					dst = FreeImage_ConvertToRGBF(mem, src);
					break;
				case FIT_RGBAF:
					break;
			}
		...
		case FIT_UINT16:
			switch(dst_type) {
				case FIT_BITMAP:
					dst = FreeImage_ConvertToStandardType(mem, src, scale_linear);
					break;
				case FIT_INT16:
					break;
				case FIT_UINT32:
				...
				case FIT_FLOAT:
					...
					break;
				case FIT_DOUBLE:
					dst = convertUShortToDouble.convert(mem, src, dst_type);
					break;
				case FIT_COMPLEX:
					dst = convertUShortToComplex.convert(mem, src);
					break;
				case FIT_RGB16:
					dst = FreeImage_ConvertToRGB16(mem, src);
					break;
		...
		case FIT_INT16:
			switch(dst_type) {
				...
				case FIT_INT32:
					break;
				case FIT_FLOAT:
					dst = convertShortToFloat.convert(mem, src, dst_type);
					break;
				case FIT_DOUBLE:
					dst = convertShortToDouble.convert(mem, src, dst_type);
					break;
				case FIT_COMPLEX:
					dst = convertShortToComplex.convert(mem, src);
					break;
				case FIT_RGB16:
					break;
				case FIT_RGBA16:
		...
		case FIT_UINT32:
			switch(dst_type) {
				...
				case FIT_INT32:
					break;
				case FIT_FLOAT:
					dst = convertULongToFloat.convert(mem, src, dst_type);
					break;
				case FIT_DOUBLE:
					dst = convertULongToDouble.convert(mem, src, dst_type);
					break;
				case FIT_COMPLEX:
					dst = convertULongToComplex.convert(mem, src);
					break;
				case FIT_RGB16:
					break;
				case FIT_RGBA16:
		...
		case FIT_INT32:
			switch(dst_type) {
				...
				case FIT_UINT32:
					break;
				case FIT_FLOAT:
					dst = convertLongToFloat.convert(mem, src, dst_type);
					break;
				case FIT_DOUBLE:
					dst = convertLongToDouble.convert(mem, src, dst_type);
					break;
				case FIT_COMPLEX:
					dst = convertLongToComplex.convert(mem, src);
					break;
				case FIT_RGB16:
					break;
				case FIT_RGBA16:
		...
		case FIT_FLOAT:
			switch(dst_type) {
				...
				case FIT_INT32:
					break;
				case FIT_DOUBLE:
					dst = convertFloatToDouble.convert(mem, src, dst_type);
					break;
				case FIT_COMPLEX:
					dst = convertFloatToComplex.convert(mem, src);
					break;
				case FIT_RGB16:
					break;
				case FIT_RGBA16:
		...
		case FIT_DOUBLE:
			switch(dst_type) {
				...
				case FIT_FLOAT:
					break;
				case FIT_COMPLEX:
					dst = convertDoubleToComplex.convert(mem, src);
					break;
				case FIT_RGB16:
					break;
				case FIT_RGBA16:
		...
		case FIT_RGB16:
			switch(dst_type) {
				case FIT_BITMAP:
					dst = FreeImage_ConvertTo24Bits(mem, src);
					break;
				case FIT_UINT16:
					dst = FreeImage_ConvertToUINT16(mem, src);
					break;
		...
		case FIT_RGBA16:
			switch(dst_type) {
				case FIT_BITMAP:
					dst = FreeImage_ConvertTo32Bits(mem, src);
					break;
				case FIT_UINT16:
					dst = FreeImage_ConvertToUINT16(mem, src);
					break;
