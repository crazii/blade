...
static BOOL 
rgbe_ReadPixels_RLE(FreeImageIO *io, FreeImageMem* mem, fi_handle handle, FIRGBF *data, int scanline_width, unsigned num_scanlines) {
	...
	while(num_scanlines > 0) {
		if(io->read_proc(rgbe, 1, sizeof(rgbe), handle) < 1) {
			FreeImage_Free(mem->temp_mem, scanline_buffer);
			return rgbe_Error(rgbe_read_error,NULL);
		}
		...
		if(scanline_buffer == NULL) {
			scanline_buffer = (BYTE*)FreeImage_Malloc(mem->temp_mem, sizeof(BYTE) * 4 * scanline_width);
			if(scanline_buffer == NULL) {
...
static BOOL 
rgbe_WritePixels_RLE(FreeImageIO *io, FreeImageMem* mem, fi_handle handle, FIRGBF *data, unsigned scanline_width, unsigned num_scanlines) {
	BYTE rgbe[4];
	BYTE *buffer;
	...
	buffer = (BYTE*)FreeImage_Malloc(mem->temp_mem, sizeof(BYTE) * 4 * scanline_width);
	if (buffer == NULL) {
	...
	while(num_scanlines-- > 0) {
		...
		if(io->write_proc(rgbe, sizeof(rgbe), 1, handle) < 1) {
			FreeImage_Malloc(mem->temp_mem, buffer);
			return rgbe_Error(rgbe_write_error, NULL);
		}
