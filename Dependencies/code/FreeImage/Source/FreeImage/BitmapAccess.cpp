...
void* FreeImage_Internal_Aligned_Malloc(size_t amount, size_t alignment) {
	assert(alignment == FIBITMAP_ALIGNMENT);
	return _aligned_malloc(amount, alignment);
}

void FreeImage_Iternal_Aligned_Free(void* mem) {
	_aligned_free(mem);
}
...
FreeImage_AllocateHeaderT(FreeImageMem* mem, BOOL header_only, FREE_IMAGE_TYPE type, int width, int height, int bpp, unsigned red_mask, unsigned green_mask, unsigned blue_mask) {
	FIBITMAP *bitmap = (FIBITMAP *)FreeImage_Malloc(mem->normal_mem,sizeof(FIBITMAP));
	...
	if (bitmap != NULL) {
		...
		switch(type) {
			...
			case FIT_RGBAF:
				...
				break;
			default:
				FreeImage_Free(mem->normal_mem, bitmap);
				return NULL;
		}
		...
		bitmap->data = (BYTE *)FreeImage_AlignMalloc(mem->normal_mem, dib_size * sizeof(BYTE), FIBITMAP_ALIGNMENT);
		...
		if (bitmap->data != NULL) {
			...
			fih->metadata = new( FreeImage_Malloc(mem->normal_mem, sizeof(METADATAMAP) ) ) METADATAMAP;
...
FIBITMAP * DLL_CALLCONV
FreeImage_AllocateHeader(FreeImageMem* mem, BOOL header_only, int width, int height, int bpp, unsigned red_mask, unsigned green_mask, unsigned blue_mask) {
	return FreeImage_AllocateHeaderT(mem, header_only, FIT_BITMAP, width, height, bpp, red_mask, green_mask, blue_mask);
}

FIBITMAP * DLL_CALLCONV
FreeImage_Allocate(FreeImageMem* mem, int width, int height, int bpp, unsigned red_mask, unsigned green_mask, unsigned blue_mask) {
	return FreeImage_AllocateHeaderT(mem, FALSE, FIT_BITMAP, width, height, bpp, red_mask, green_mask, blue_mask);
}

FIBITMAP * DLL_CALLCONV
FreeImage_AllocateT(FreeImageMem* mem, FREE_IMAGE_TYPE type, int width, int height, int bpp, unsigned red_mask, unsigned green_mask, unsigned blue_mask) {
	return FreeImage_AllocateHeaderT(mem, FALSE, type, width, height, bpp, red_mask, green_mask, blue_mask);
}

void DLL_CALLCONV
FreeImage_Unload(FreeImageMem* mem, FIBITMAP *dib) {
	if (NULL != dib) {	
		if (NULL != dib->data) {
			// delete possible icc profile ...
			if (FreeImage_GetICCProfile(dib)->data)
				FreeImage_Free(mem->normal_mem,FreeImage_GetICCProfile(dib)->data);
			...
			for(METADATAMAP::iterator i = (*metadata).begin(); i != (*metadata).end(); i++) {
				...
				if(tagmap) {
					...
					//delete tagmap;
					tagmap->~TAGMAP();
					FreeImage_Free(mem->normal_mem,tagmap);
				}
			}

			//delete metadata;
			metadata->~METADATAMAP();
			FreeImage_Free(mem->normal_mem,metadata);

			// delete embedded thumbnail
			FreeImage_Unload(mem, FreeImage_GetThumbnail(dib));

			// delete bitmap ...
			FreeImage_AlignFree(mem->normal_mem, dib->data);
		}
		FreeImage_Free(mem->normal_mem,dib);		// ... and the wrapper
	}
}
...
FIBITMAP * DLL_CALLCONV
FreeImage_Clone(FreeImageMem* mem, FIBITMAP *dib) {
	if(!dib) return NULL;
	...
	// allocate a new dib
	FIBITMAP *new_dib = FreeImage_AllocateHeaderT(mem, header_only, FreeImage_GetImageType(dib), width, height, bpp, 
	...
	if (new_dib) {
		...
		FreeImage_CreateICCProfile(mem, new_dib, src_iccProfile->data, src_iccProfile->size);
		...
		for(METADATAMAP::iterator i = (*src_metadata).begin(); i != (*src_metadata).end(); i++) {
			...
			if(src_tagmap) {
				...
				TAGMAP *dst_tagmap = new( FreeImage_Malloc(mem->normal_mem,sizeof(TAGMAP)) ) TAGMAP();

				if(dst_tagmap) {
					// fill the model
					for(TAGMAP::iterator j = src_tagmap->begin(); j != src_tagmap->end(); j++) {
						std::string dst_key = (*j).first;
						FITAG *dst_tag = FreeImage_CloneTag(mem, (*j).second );
		...
		// copy the thumbnail
		FreeImage_SetThumbnail(mem, new_dib, FreeImage_GetThumbnail(dib));
...
BOOL DLL_CALLCONV
//FreeImage_SetThumbnail(FreeImageMem* mem_old, FreeImageMem* mem_new, FIBITMAP *dib, FIBITMAP *thumbnail) {
FreeImage_SetThumbnail(FreeImageMem* mem, FIBITMAP *dib, FIBITMAP *thumbnail) {
	if(dib == NULL) {
		return FALSE;
	}
	...
	FreeImage_Unload(mem,currentThumbnail);

	((FREEIMAGEHEADER *)dib->data)->thumbnail = FreeImage_HasPixels(thumbnail) ? FreeImage_Clone(mem, thumbnail) : NULL;
 ...
 */
void DLL_CALLCONV
FreeImage_SetTransparentIndex(FreeImageMem* mem, FIBITMAP *dib, int index) {
	if (dib) {
		int count = FreeImage_GetColorsUsed(dib);
		if (count) {
			BYTE *new_tt = (BYTE *)FreeImage_Malloc(mem->temp_mem, count * sizeof(BYTE));
			memset(new_tt, 0xFF, count);
			if ((index >= 0) && (index < count)) {
				new_tt[index] = 0x00;
			}
			FreeImage_SetTransparencyTable(dib, new_tt, count);
			FreeImage_Free(mem->temp_mem, new_tt);
		}
	}
}
...
FIICCPROFILE * DLL_CALLCONV
FreeImage_CreateICCProfile(FreeImageMem* mem, FIBITMAP *dib, void *data, long size) {
	// clear the profile but preserve profile->flags
	FreeImage_DestroyICCProfile(mem, dib);
	// create the new profile
	FIICCPROFILE *profile = FreeImage_GetICCProfile(dib);
	if(size && profile) {
		profile->data = FreeImage_Malloc(mem->normal_mem, size);
		if(profile->data) {
...
void DLL_CALLCONV
FreeImage_DestroyICCProfile(FreeImageMem* mem, FIBITMAP *dib) {
	FIICCPROFILE *profile = FreeImage_GetICCProfile(dib);
	if(profile) {
		if (profile->data) {
			FreeImage_Free(mem->normal_mem, profile->data);
		}
		// clear the profile but preserve profile->flags
		profile->data = NULL;
		profile->size = 0;
...
FreeImage_FindFirstMetadata(FreeImageMem* mem, FREE_IMAGE_MDMODEL model, FIBITMAP *dib, FITAG **tag) {
	if(!dib)
		return NULL;
	...
	if(tagmap) {
		// allocate a handle
		FIMETADATA 	*handle = (FIMETADATA *)FreeImage_Malloc(mem->normal_mem, sizeof(FIMETADATA));
		if(handle) {
			// calculate the size of a METADATAHEADER
			int header_size = sizeof(METADATAHEADER);

			handle->data = (BYTE *)FreeImage_Malloc(mem->normal_mem, header_size * sizeof(BYTE));
			...
			FreeImage_Free(mem->normal_mem, handle);
		}
	}
...
void DLL_CALLCONV 
FreeImage_FindCloseMetadata(FreeImageMem* mem, FIMETADATA *mdhandle) {
	if (NULL != mdhandle) {	// delete the handle
		if (NULL != mdhandle->data) {
			FreeImage_Free(mem->normal_mem, mdhandle->data);
		}
		FreeImage_Free(mem->normal_mem, mdhandle);		// ... and the wrapper
	}
}
...
BOOL DLL_CALLCONV
FreeImage_CloneMetadata(FreeImageMem* mem, FIBITMAP *dst, FIBITMAP *src) {
	...
	for(METADATAMAP::iterator i = (*src_metadata).begin(); i != (*src_metadata).end(); i++) {
		...
		if(src_tagmap) {
			if( dst_metadata->find(model) != dst_metadata->end() ) {
				// destroy dst model
				FreeImage_SetMetadata((FREE_IMAGE_MDMODEL)model, mem, dst, NULL, NULL);
			}
...
BOOL DLL_CALLCONV 
FreeImage_SetMetadata(FREE_IMAGE_MDMODEL model, FreeImageMem* mem, FIBITMAP *dib, const char *key, FITAG *tag) {
	if(!dib) 
		return FALSE;
	...
	if(key != NULL) {
		...
		if(!tagmap) {
			...
			tagmap = new( FreeImage_Malloc(mem->normal_mem,sizeof(TAGMAP)) ) TAGMAP();
		...
		if(tag) {
			...
			if(old_tag) {
				FreeImage_DeleteTag(mem, old_tag);
			}

			// create a new tag
			(*tagmap)[key] = FreeImage_CloneTag(mem, tag);
		}
		else {
			// delete existing tag
