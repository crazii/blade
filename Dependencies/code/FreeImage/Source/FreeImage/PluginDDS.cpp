...
static FIBITMAP *
LoadRGB (DDSURFACEDESC2 &desc, FreeImageIO *io, FreeImageMem* mem, fi_handle handle, int page, int flags, void *data) {
	...
	// allocate a new dib
	FIBITMAP *dib = FreeImage_Allocate (&tmem, width, height, bpp, desc.ddpfPixelFormat.dwRBitMask,
	...
	if (!(desc.ddpfPixelFormat.dwFlags & DDPF_ALPHAPIXELS) && bpp == 32) {
		...
		FIBITMAP *old = dib;
		dib = FreeImage_ConvertTo24Bits (mem, old);
		FreeImage_Unload (&tmem,old);
	}
	else
	{
		FIBITMAP* old = dib;
		dib = FreeImage_Clone(mem,old);
		FreeImage_Unload(&tmem,old);
	}
	return dib;
}
...
LoadDXT_Helper (FreeImageIO *io, FreeImageMem* mem, fi_handle handle, int page, int flags, void *data, FIBITMAP *dib, int width, int height, int line) {
	typedef typename DECODER::INFO INFO;
	typedef typename INFO::Block Block;

	Block *input_buffer = (Block*)FreeImage_Malloc(mem->temp_mem,sizeof(Block)*(width + 3) / 4 );//new(std::nothrow) Block[(width + 3) / 4];
	...
	//delete [] input_buffer;
	::FreeImage_Free(mem->temp_mem, input_buffer);
}

static FIBITMAP *
LoadDXT (int type, DDSURFACEDESC2 &desc, FreeImageIO *io, FreeImageMem* mem, fi_handle handle, int page, int flags, void *data) {
	int width = (int)desc.dwWidth & ~3;
	int height = (int)desc.dwHeight & ~3;

	// allocate a 32-bit dib
	FIBITMAP *dib = FreeImage_Allocate (mem, width, height, 32, FI_RGBA_RED_MASK, FI_RGBA_GREEN_MASK, FI_RGBA_BLUE_MASK);
	if (dib == NULL)
		return NULL;
	...
	switch (type) {
		case 1:
			LoadDXT_Helper <DXT_BLOCKDECODER_1> (io, mem, handle, page, flags, data, dib, width, height, line);
			break;
		case 3:
			LoadDXT_Helper <DXT_BLOCKDECODER_3> (io, mem, handle, page, flags, data, dib, width, height, line);
			break;
		case 5:
			LoadDXT_Helper <DXT_BLOCKDECODER_5> (io, mem, handle, page, flags, data, dib, width, height, line);
			break;
	}
...
Load(FreeImageIO *io, FreeImageMem* mem, fi_handle handle, int page, int flags, void *data) {
	...
	if (header.surfaceDesc.ddpfPixelFormat.dwFlags & DDPF_RGB) {
		dib = LoadRGB (header.surfaceDesc, io, mem, handle, page, flags, data);
	}
	else if (header.surfaceDesc.ddpfPixelFormat.dwFlags & DDPF_FOURCC) {
		switch (header.surfaceDesc.ddpfPixelFormat.dwFourCC) {
			case FOURCC_DXT1:
				dib = LoadDXT (1, header.surfaceDesc, io, mem, handle, page, flags, data);
				break;
			case FOURCC_DXT3:
				dib = LoadDXT (3, header.surfaceDesc, io, mem, handle, page, flags, data);
				break;
			case FOURCC_DXT5:
				dib = LoadDXT (5, header.surfaceDesc, io, mem, handle, page, flags, data);
				break;
		}
	}
	return dib;
