...
FreeImage_TmoReinhard05Ex(FreeImageMem* mem, FIBITMAP *src, double intensity, double contrast, double adaptation, double color_correction) {
	...
	dib = FreeImage_ConvertToRGBF(&tmem, src);
	if(!dib) return NULL;

	// get the Luminance channel
	Y = ConvertRGBFToY(&tmem, dib);
	if(!Y) {
...
*/
FIBITMAP* DLL_CALLCONV 
FreeImage_TmoReinhard05(FreeImageMem* mem, FIBITMAP *src, double intensity, double contrast) {
	return FreeImage_TmoReinhard05Ex(mem, src, intensity, contrast, 1, 0);
}
