...
WuQuantizer::WuQuantizer(FreeImageMem* mem, FIBITMAP *dib) {
	memobject = mem;
	...
	// Allocate 3D arrays
	gm2 = (float*)FreeImage_Malloc(mem->temp_mem, SIZE_3D * sizeof(float));
	wt = (LONG*)FreeImage_Malloc(mem->temp_mem, SIZE_3D * sizeof(LONG));
	mr = (LONG*)FreeImage_Malloc(mem->temp_mem, SIZE_3D * sizeof(LONG));
	mg = (LONG*)FreeImage_Malloc(mem->temp_mem, SIZE_3D * sizeof(LONG));
	mb = (LONG*)FreeImage_Malloc(mem->temp_mem, SIZE_3D * sizeof(LONG));

	// Allocate Qadd
	Qadd = (WORD *)FreeImage_Malloc(mem->temp_mem, sizeof(WORD) * width * height);

	if(!gm2 || !wt || !mr || !mg || !mb || !Qadd) {
		if(gm2)	FreeImage_Free(mem->temp_mem, gm2);
		if(wt)	FreeImage_Free(mem->temp_mem, wt);
		if(mr)	FreeImage_Free(mem->temp_mem, mr);
		if(mg)	FreeImage_Free(mem->temp_mem, mg);
		if(mb)	FreeImage_Free(mem->temp_mem, mb);
		if(Qadd)  FreeImage_Free(mem->temp_mem, Qadd);
...
WuQuantizer::~WuQuantizer() {
	if(gm2)	FreeImage_Free(memobject->temp_mem, gm2);
	if(wt)	FreeImage_Free(memobject->temp_mem, wt);
	if(mr)	FreeImage_Free(memobject->temp_mem, mr);
	if(mg)	FreeImage_Free(memobject->temp_mem, mg);
	if(mb)	FreeImage_Free(memobject->temp_mem, mb);
	if(Qadd)  FreeImage_Free(memobject->temp_mem, Qadd);
}
...
FIBITMAP *
WuQuantizer::Quantize(FreeImageMem* mem, int PaletteSize, int ReserveSize, RGBQUAD *ReservePalette) {
	BYTE *tag = NULL;
	...
	try {
		...
		FreeImage_Free(mem->temp_mem, gm2);

		gm2 = NULL;

		// Allocate a new dib

		FIBITMAP *new_dib = FreeImage_Allocate(mem, width, height, 8);
		...
		tag = (BYTE*) FreeImage_Malloc(mem->temp_mem,SIZE_3D * sizeof(BYTE));
		if (tag == NULL) {
			throw FI_MSG_ERROR_MEMORY;
		}
		...
		FreeImage_Free(mem->temp_mem,tag);
