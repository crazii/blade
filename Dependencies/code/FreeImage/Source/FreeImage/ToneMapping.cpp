...
FreeImage_ToneMapping(FreeImageMem* mem, FIBITMAP *dib, FREE_IMAGE_TMO tmo, double first_param, double second_param) {
		...
		switch(tmo) {
			...
			case FITMO_DRAGO03:
				if((first_param == 0) && (second_param == 0)) {
					...
					return FreeImage_TmoDrago03(mem, dib, 2.2, 0);
				} else {
					// use user's value
					return FreeImage_TmoDrago03(mem, dib, first_param, second_param);
				}
				break;
			...
			case FITMO_REINHARD05:
				if((first_param == 0) && (second_param == 0)) {
					...
					return FreeImage_TmoReinhard05(mem, dib, 0, 0);
				} else {
					// use user's value
					return FreeImage_TmoReinhard05(mem, dib, first_param, second_param);
				}
				break;
			...
			case FITMO_FATTAL02:
				if((first_param == 0) && (second_param == 0)) {
					...
					return FreeImage_TmoFattal02(mem, dib, 0.5, 0.85);
				} else {
					// use user's value
					return FreeImage_TmoFattal02(mem, dib, first_param, second_param);
				}
				break;
		}
	}
