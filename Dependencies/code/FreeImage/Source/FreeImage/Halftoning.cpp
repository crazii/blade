...
//          *   7
//      3   5   1     (1/16)
static FIBITMAP* FloydSteinberg(FreeImageMem* mem, FIBITMAP *dib) {
...
#define INITERR(X, Y) (((int) X) - (((int) Y) ? WHITE : BLACK) + ((WHITE/2)-((int)X)) / 2)
	...
	new_dib = FreeImage_Allocate(mem, width, height, 8);
	if(NULL == new_dib) return NULL;

	// allocate space for error arrays
	int *lerr = (int*)FreeImage_Malloc(mem->temp_mem, width * sizeof(int));
	int *cerr = (int*)FreeImage_Malloc(mem->temp_mem,width * sizeof(int));
	...
	FreeImage_Free(mem->temp_mem,lerr);
	FreeImage_Free(mem->temp_mem,cerr);
...
//
static FIBITMAP* OrderedDispersedDot(FreeImageMem* mem, FIBITMAP *dib, int order) {
	int x, y;
	int width, height;
	BYTE *bits, *new_bits;
	FIBITMAP *new_dib = NULL;
	...
	BYTE *matrix = (BYTE*)FreeImage_Malloc(mem->temp_mem,l*l * sizeof(BYTE));
	for(int i = 0; i < l*l; i++) {
	...
	FreeImage_Free(mem->temp_mem,matrix);
...
//
static FIBITMAP* OrderedClusteredDot(FreeImageMem* mem, FIBITMAP *dib, int order) {
...
FreeImage_Dither(FreeImageMem* mem, FIBITMAP *dib, FREE_IMAGE_DITHER algorithm) {
	...
	if(bpp == 1) {
		...
		FIBITMAP *new_dib = FreeImage_Clone(mem, dib);
		if(NULL == new_dib) return NULL;
	...
	switch(bpp) {
		case 8:
			if(FreeImage_GetColorType(dib) == FIC_MINISBLACK) {
				...
				input = FreeImage_ConvertToGreyscale(&tmem, dib);
			} 
			break;
		case 4:
		case 16:
	...
	switch(algorithm) {
		case FID_FS:
			dib8 = FloydSteinberg(&tmem, input);
			break;
		case FID_BAYER4x4:
			dib8 = OrderedDispersedDot(&tmem, input, 2);
			break;
		case FID_BAYER8x8:
			dib8 = OrderedDispersedDot(&tmem, input, 3);
			break;
		case FID_BAYER16x16:
			dib8 = OrderedDispersedDot(&tmem, input, 4);
			break;
		case FID_CLUSTER6x6:
			dib8 = OrderedClusteredDot(&tmem, input, 3);
			break;
		case FID_CLUSTER8x8:
			dib8 = OrderedClusteredDot(&tmem, input, 4);
			break;
		case FID_CLUSTER16x16:
			dib8 = OrderedClusteredDot(&tmem, input, 8);
			break;
	}
	if(input != dib) {
		FreeImage_Unload(&tmem, input);
	}
	...
	// Convert to 1-bit
	FIBITMAP *new_dib = FreeImage_Threshold(mem, dib8, 128);
	FreeImage_Unload(&tmem, dib8);

	// copy metadata from src to dst
	FreeImage_CloneMetadata(mem, new_dib, dib);
...
//
FIBITMAP * DLL_CALLCONV
FreeImage_Threshold(FreeImageMem* mem, FIBITMAP *dib, BYTE T) {
	FIBITMAP *dib8 = NULL;
	...
	switch(bpp) {
		case 8:
			if(FreeImage_GetColorType(dib) == FIC_MINISBLACK) {
				...
				dib8 = FreeImage_ConvertToGreyscale(&tmem, dib);
			} 
			break;
		case 4:
		case 16:
	...
	FIBITMAP *new_dib = FreeImage_Allocate(mem, width, height, 1);
	if(NULL == new_dib) return NULL;
	// Build a monochrome palette
	RGBQUAD *pal = FreeImage_GetPalette(new_dib);
	pal[0].rgbRed = pal[0].rgbGreen = pal[0].rgbBlue = 0;
