...
int DLL_CALLCONV
FreeImage_InternalGetPageCount(FreeImageMem* mem, FIMULTIBITMAP *bitmap) {	
	if (bitmap) {
		if (((MULTIBITMAPHEADER *)bitmap->data)->handle) {
			...
			header->io->seek_proc(header->handle, 0, SEEK_SET);
   			...
   			void *data = FreeImage_Open(header->node, header->io, mem, header->handle, TRUE);
			...
			FreeImage_Close(header->node, header->io, mem, header->handle, data);
...
FreeImage_OpenMultiBitmap(FREE_IMAGE_FORMAT fif, FreeImageMem* mem, const char *filename, BOOL create_new, BOOL read_only, BOOL keep_cache_in_memory, int flags) {
	...
	try {
		...
		if (list) {
			...
			if (node) {
				...
				header->page_count = FreeImage_InternalGetPageCount(mem, bitmap.get());
...
FIMULTIBITMAP * DLL_CALLCONV
FreeImage_OpenMultiBitmapFromHandle(FREE_IMAGE_FORMAT fif, FreeImageIO *io, FreeImageMem* mem, fi_handle handle, int flags) {
	try {
...
BOOL DLL_CALLCONV
FreeImage_SaveMultiBitmapToHandle(FREE_IMAGE_FORMAT fif, FIMULTIBITMAP *bitmap, FreeImageIO *io, FreeImageMem* mem, fi_handle handle, int flags) {
	...
	if (list) {
		...
		if(node) {
			...
			// dst data
			void *data = FreeImage_Open(node, io, mem, handle, FALSE);
			// src data
			void *data_read = NULL;
			...
			if(header->handle) {
				// open src
				header->io->seek_proc(header->handle, 0, SEEK_SET);
				data_read = FreeImage_Open(header->node, header->io, mem, header->handle, TRUE);
			}
					...
					switch((*i)->m_type) {
						case BLOCK_CONTINUEUS:
						{
							...
							for (int j = block->m_start; j <= block->m_end; j++) {
								...
								FIBITMAP *dib = header->node->m_plugin->load_proc(header->io, &tmem, header->handle, j, header->load_flags, data_read);
								
								// save the data
								success = node->m_plugin->save_proc(io, &tmem, dib, handle, count, flags, data);
								count++;
								
								FreeImage_Unload(&tmem,dib);
							}
						...
						case BLOCK_REFERENCE:
						{
							...
							BYTE *compressed_data = (BYTE*)FreeImage_Malloc(mem->temp_mem,ref->m_size * sizeof(BYTE));
							
							header->m_cachefile->readFile((BYTE *)compressed_data, ref->m_reference, ref->m_size);
							
							// uncompress the data
							
							FIMEMORY *hmem = FreeImage_OpenMemory(&tmem, compressed_data, ref->m_size);
							FIBITMAP *dib = FreeImage_LoadFromMemory(header->cache_fif, &tmem, hmem, 0);
							FreeImage_CloseMemory(&tmem, hmem);
							
							// get rid of the buffer
							FreeImage_Free(mem->temp_mem,compressed_data);
			...
			FreeImage_Close(header->node, header->io, mem, header->handle, data_read);

			FreeImage_Close(node, io, mem, handle, data); 
...
BOOL DLL_CALLCONV
FreeImage_CloseMultiBitmap(FreeImageMem* mem, FIMULTIBITMAP *bitmap, int flags) {
	if (bitmap) {
		BOOL success = TRUE;
		...
		if (bitmap->data) {
			...
			if (header->changed && header->m_filename) {
				try {
					...
					if (f == NULL) {
						...
						success = FreeImage_SaveMultiBitmapToHandle(header->fif, bitmap, header->io, mem, (fi_handle)f, flags);
			...
			while (!header->locked_pages.empty()) {
				FreeImage_Unload(mem, header->locked_pages.begin()->first);
...
void DLL_CALLCONV
FreeImage_AppendPage(FreeImageMem* mem, FIMULTIBITMAP *bitmap, FIBITMAP *data) {
	if ((bitmap) && (data)) {
		MULTIBITMAPHEADER *header = FreeImage_GetMultiBitmapHeader(bitmap);
		...
		if ((!header->read_only) && (header->locked_pages.empty())) {
			...
			FIMEMORY *hmem = FreeImage_OpenMemory(mem);
			// save the file to memory
			FreeImage_SaveToMemory(header->cache_fif, mem, data, hmem, 0);
			...
			FreeImage_CloseMemory(mem, hmem);
...
void DLL_CALLCONV
FreeImage_InsertPage(FreeImageMem* mem, FIMULTIBITMAP *bitmap, int page, FIBITMAP *data) {
	if ((bitmap) && (data)) {
...
FreeImage_LockPage(FreeImageMem* mem, FIMULTIBITMAP *bitmap, int page) {
	if (bitmap) {
		MULTIBITMAPHEADER *header = FreeImage_GetMultiBitmapHeader(bitmap);
		...
		if (data != NULL) {
			FIBITMAP *dib = (header->node->m_plugin->load_proc != NULL) ? header->node->m_plugin->load_proc(header->io, mem, header->handle, page, header->load_flags, data) : NULL;
...
void DLL_CALLCONV
FreeImage_UnlockPage(FreeImageMem* mem, FIMULTIBITMAP *bitmap, FIBITMAP *page, BOOL changed) {
	if ((bitmap) && (page)) {
		MULTIBITMAPHEADER *header = FreeImage_GetMultiBitmapHeader(bitmap);
		...
		if (header->locked_pages.find(page) != header->locked_pages.end()) {
			...
			if (changed && !header->read_only) {
				...
				FreeImage_SaveToMemory(header->cache_fif, mem, page, hmem, 0);
				// get the buffer from the memory stream
				FreeImage_AcquireMemory(hmem, &compressed_data, &compressed_size);
...
FIMULTIBITMAP * DLL_CALLCONV
FreeImage_LoadMultiBitmapFromMemory(FREE_IMAGE_FORMAT fif, FreeImageMem* mem, FIMEMORY *stream, int flags) {
	BOOL read_only = FALSE;	// modifications (if any) will be stored into the memory cache
			...
			if (io) {
				...
				if (bitmap) {
					...
					if (header) {
						...
						header->page_count = FreeImage_InternalGetPageCount(mem, bitmap);
...
BOOL DLL_CALLCONV
FreeImage_SaveMultiBitmapToMemory(FreeImageMem* mem, FREE_IMAGE_FORMAT fif, FIMULTIBITMAP *bitmap, FIMEMORY *stream, int flags) {
	if (stream && stream->data) {
		FreeImageIO io;
		SetMemoryIO(&io);

		return FreeImage_SaveMultiBitmapToHandle(fif, bitmap, &io, mem, (fi_handle)stream, flags);
	}
