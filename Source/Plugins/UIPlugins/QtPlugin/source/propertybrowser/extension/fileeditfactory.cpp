...
void FileEditFactory::connectPropertyManager(FilePathManager *manager)
{
	...
	connect(manager, SIGNAL(iconChanged(QtProperty *, const QIcon &)),
		this, SLOT(slotIconChanged(QtProperty *, const QIcon &)));
}
        ...
        QtProperty *property, QWidget *parent)
{
	...
	editor->setType(manager->type(property));
	editor->setIcon(manager->icon(property));
	...
	connect(editor, SIGNAL(callbackInvoked(FileEdit* , QString& , bool&)),
		this, SLOT(slotCallback(FileEdit* , QString& , bool&)));
    return editor;
}
...
void FileEditFactory::disconnectPropertyManager(FilePathManager *manager)
{
	...
	disconnect(manager, SIGNAL(readOnlyChanged(QtProperty *, bool)),
		this, SLOT(slotReadOnlyChanged(QtProperty *, bool)));
	disconnect(manager, SIGNAL(iconChanged(QtProperty *, const QIcon &)),
		this, SLOT(slotIconChanged(QtProperty *, const QIcon &)));
}
...
void FileEditFactory::slotPropertyChanged(QtProperty *property,
                const QString &value)
{
	QMap<QtProperty *, QList<FileEdit *> >::iterator i = theCreatedEditors.find(property);
    if (i == theCreatedEditors.end())
        return;

    QList<FileEdit *>& editors = *i;
...
void FileEditFactory::slotReadOnlyChanged(QtProperty* property, bool readOnly)
{
	QMap<QtProperty *, QList<FileEdit *> >::iterator i = theCreatedEditors.find(property);
	if (i == theCreatedEditors.end())
		return;
	...
	QListIterator<FileEdit *> itEditor(editors);
	while (itEditor.hasNext())
		itEditor.next()->setReadOnly(readOnly);
}

void FileEditFactory::slotIconChanged(QtProperty* property, const QIcon& icon)
{
	QMap<QtProperty *, QList<FileEdit *> >::iterator i = theCreatedEditors.find(property);
	if (i == theCreatedEditors.end())
		return;
	...
	QListIterator<FileEdit *> itEditor(editors);
	while (itEditor.hasNext())
		itEditor.next()->setIcon(icon);
}

void FileEditFactory::slotTypeChanged(QtProperty* property, EFilePropertyType type)
{
	QMap<QtProperty *, QList<FileEdit *> >::iterator i = theCreatedEditors.find(property);
	if (i == theCreatedEditors.end())
		return;
...
void FileEditFactory::slotCallback(FileEdit* edit, QString& path, bool& succeeded)
{
	QMap<FileEdit *, QtProperty *>::iterator i = theEditorToProperty.find(edit);
	if(i == theEditorToProperty.end())
		return;

	emit propertyCallbackInvoked(*i, path, succeeded);
}
