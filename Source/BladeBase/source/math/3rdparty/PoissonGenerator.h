...
/********************************************************************
created:	2017/12/13
filename: 	PoissonGenerator.h
author:		https://github.com/corporateshark/poisson-disk-generator
purpose:	
*********************************************************************/
#include <BladePCH.h>
...
public:
	...
	float RandomFloat()
	{
		return Blade::Math::Random(0.0f, 1.0f);
	}

	int RandomInt( int Max )
	{
		return Blade::Math::Random(0, Max);
	}
};
...
struct sGrid
{
	sGrid( int W, int H, float CellSize )
		...
		, m_CellSize( CellSize )
	{
		m_Grid.resize( (size_t)m_H );

		for (Blade::TempVector< Blade::TempVector<sPoint> >::iterator i = m_Grid.begin(); i != m_Grid.end(); i++ ) { i->resize( (size_t)m_W ); }
	}
	void Insert( const sPoint& P )
	{
		sGridPoint G = ImageToGrid( P, m_CellSize );
		m_Grid[(size_t)G.x ][(size_t)G.y ] = P;
	}
	bool IsInNeighbourhood( sPoint Point, float MinDist, float CellSize )
	{
		...
		for ( int i = G.x - D; i < G.x + D; i++ )
		{
			for ( int j = G.y - D; j < G.y + D; j++ )
			{
				...
				{
					sPoint P = m_Grid[(size_t)i ][(size_t)j ];
...
private:
	...
	Blade::TempVector< Blade::TempVector<sPoint> > m_Grid;
};

template <typename PRNG>
sPoint PopRandom(Blade::TempVector<sPoint>& Points, PRNG& Generator )
{
	const int Idx = Generator.RandomInt( (int)Points.size()-1 );
	const sPoint P = Points[ (size_t)Idx ];
...
**/
template <typename PRNG>
Blade::TempVector<sPoint> GeneratePoissonPoints(
	size_t NumPoints,
	PRNG& Generator,
...
)
{
	if ( MinDist < 0.0f )
	{
		MinDist = std::sqrt( float(NumPoints) ) / float(NumPoints);
	}

	Blade::TempVector<sPoint> SamplePoints;
	Blade::TempVector<sPoint> ProcessList;
...
template <bool CIRCLE, typename PRNG>
void GeneratePoissonPoints2(
	Blade::Vector2* outPoints,
	size_t NumPoints,
	PRNG& Generator,
	const Blade::Vector2& origin,
	float scale,
	int NewPointsCount = 30,
	float MinDist = -1.0f
)
{
	...
	size_t GenCount = 0;
	// update containers
	outPoints[GenCount++] = reinterpret_cast<Blade::Vector2&>(FirstPoint) * scale + origin;
	ProcessList.push_back(FirstPoint);
	Grid.Insert(FirstPoint);

	// generate new points for each point in the queue
	while (!ProcessList.empty() && GenCount < NumPoints)
	{
		sPoint Point = PopRandom<PRNG>(ProcessList, Generator);
